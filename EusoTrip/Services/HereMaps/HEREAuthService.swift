...
actor HEREAuthService {
    ...
    private func exchange() async throws -> CachedToken {
        ...
        let headerPieces: [String] = headerParams.map { pair -> String in
            let encodedValue = Self.percentEncode(pair.1)
            return "\(pair.0)=\"\(encodedValue)\""
        }
    ...
    static func sign(method: String,
        ...
        let encodedPairs: [(String, String)] = params.map { pair -> (String, String) in
            let k = percentEncode(pair.0)
            let v = percentEncode(pair.1)
            return (k, v)
        }
        let encoded: [(String, String)] = encodedPairs.sorted { lhs, rhs in
            if lhs.0 == rhs.0 {
                return lhs.1 < rhs.1
            }
            return lhs.0 < rhs.0
        }
        let pieces: [String] = encoded.map { pair -> String in
            return "\(pair.0)=\(pair.1)"
        }
        let paramString = pieces.joined(separator: "&")
