...
struct WalletCardTheme: Identifiable, Decodable, Equatable {
    ...
    // Decode defensively (zero-fabrication / zero decode-throw): the server
    // projection (eusoWallet.listWalletThemes) always sends id/name/kind/
    // background/foreground/label and an optional `shipNow`, but a renamed or
    // missing field must NEVER throw and blank the picker — it falls back to a
    // safe placeholder so the grid still renders the rest of the registry.
    ...
    init(from decoder: Decoder) throws {
        ...
        kind       = (try? c?.decodeIfPresent(String.self, forKey: .kind)) ?? "solid"
        background = (try? c?.decodeIfPresent(String.self, forKey: .background)) ?? "rgb(20,115,255)"
        foreground = (try? c?.decodeIfPresent(String.self, forKey: .foreground)) ?? "rgb(255,255,255)"
        label      = (try? c?.decodeIfPresent(String.self, forKey: .label)) ?? "rgb(188,211,255)"
