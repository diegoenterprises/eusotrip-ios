...
//
//  §W OFFLINE POLICY: ONLINE_ONLY(the day plan is derived from live load status and the HOS
//  clock; a cached leg could route a driver to a stop already cancelled).
...
private final class ItineraryViewModel: ObservableObject {
    ...
    func load() async {
        phase = .loading
        do {
            let schedule: ItinerarySchedule = try await EusoTripAPI.shared.queryNoInput(
                "driverMobile.getDriverSchedule"
            )
        ...
        VStack(alignment: .leading, spacing: Space.s3) {
            ...
            HStack(spacing: Space.s3) {
                ...
                MetricTile(label: "Drive left", value: driveClockLabel,
                           accent: vm.driveRemainingMin.map { $0 < 120 } == true ? Brand.warning : nil)
            }
