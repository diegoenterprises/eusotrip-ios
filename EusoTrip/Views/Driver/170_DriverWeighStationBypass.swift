...
//
//  §W OFFLINE POLICY: ONLINE_ONLY(scale open/closed and bypass eligibility are only true at the
//  moment they are read; a cached "closed" routes a driver past an open
//  scale at 65 mph).
...
struct WeighBypassView: View {
        ...
        HStack(alignment: .top) {
            ...
            VStack(alignment: .trailing, spacing: 2) {
                Text((s.status ?? "—").uppercased())
                    ...
                    .foregroundStyle(statusTint(s))
                // `prepassEnabled` is the whole point of this screen — whether
                // the driver's transponder can clear THIS scale — and it was
                // decoded and never rendered. Tri-state: unknown stays silent.
                if let bypass = s.prepassEnabled {
                    Text(bypass ? "BYPASS OK" : "NO BYPASS")
                        .font(EType.micro).tracking(0.4)
                        .foregroundStyle(bypass ? Brand.success : palette.textTertiary)
                }
            }
        }
        .padding(.vertical, 2)
    ...
    /// Station tint. Prefers the SERVER's own `statusColor` field, which was
    /// previously decoded and discarded in favour of re-deriving a colour from
    /// the free-text `status` against three guessed literals — so every
    /// unenumerated value fell to neutral, which reads as "nothing to worry
    /// about" on a scale the driver is approaching at speed.
    private func statusTint(_ s: WeighStationRow) -> Color {
        switch (s.statusColor ?? "").lowercased() {
        case "green":  return Brand.success
        case "yellow", "amber": return Brand.warning
        case "red":    return Brand.danger
        default: break
        }
        switch (s.status ?? "").lowercased() {
        case "open":   return Brand.danger      // open scale = you are pulling in
