...
//  earnings.getEarnings (per-load completed rows, the canonical driver-
//  side proc). The frame names `earnings.previewSettlement` — that proc
//  does NOT exist on the earningsRouter (verified against EusoTripAPI),
//  so it is treated as a STUB·named-gap; we wire the closest EXISTS proc
//  `earnings.getEarnings` to read the driver net + hazmat premium for
//  this closed load and surface any failure honestly through @State
//  actionError — no synthesized replies, no mock data.
//
...
struct DriverClosed: View {
    ...
    private var linehaulAmount: Double { frameLinehaul }
    ...
    private var settleRows: [SettleRow] {
        [
            ...
            SettleRow(title: "Detention · 45 min @ $90/hr",
                      amount: usd(frameDetention, signed: true), state: .billed),
            SettleRow(title: "Catalyst share · Eusotrans LLC 10%",
                      amount: usd(-frameCatalyst, signed: true), state: .netted),
    ...
    /// Read the closed-load financial breakdown.
    ///
    /// The frame's caption names `earnings.previewSettlement` — that
    /// procedure does NOT exist on the earningsRouter (verified against
    /// EusoTripAPI / `server/routers/earnings.ts`). It is a STUB·named-gap.
    /// We wire the closest EXISTS proc, `earnings.getEarnings({ period })`,
    /// which returns per-load completed rows carrying `totalPay` (driver
    /// net) and `hazmatPremium`. We match the row for the active closed
    /// load and read its real figures; any failure surfaces honestly. The
    /// frame's authored references stand in until a successful read.
    private func loadSettlement() async {
        isLoadingSettlement = true
        defer { isLoadingSettlement = false }
        do {
            let rows = try await EusoTripAPI.shared.earnings.getEarnings(period: "month")
            let match = rows.first { row in
                if let ln = row.loadNumber,
                   ln == (activeLoad?.loadNumber ?? frameLoadId) { return true }
                if let lid = lifecycle.loadId.isEmpty ? nil : lifecycle.loadId,
                   row.id == lid { return true }
                return false
            }
            if let m = match {
                settledNet = m.totalPay
                settledHazmat = m.hazmatPremium
            }
            // No matching closed-load row yet → keep the frame references;
            // do NOT fabricate a settlement figure.
        } catch {
