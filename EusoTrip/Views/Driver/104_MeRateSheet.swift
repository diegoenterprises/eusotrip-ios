...
//
//  Four-pane surface for drivers + owner-operators:
//
//    • PREVIEW — legacy Schedule-A arithmetic for comparison only. It cannot
//      award work, set a fee, bind a settlement, or become route authority.
//    • SHEETS — canonical `pricedRouteRateSheet` immutable intake, blockers,
//      exact-draft human confirmation, plus read-only legacy sheet history.
//    • RECONCILE — `listReconciliations` + `getStats`. Read-only
//      list of past statements with the running totals at the top.
//
//  Executable pricing is exclusively `pricedRoute`: the server resolves the
//  exact route plan, committed availability, policy versions, line items,
//  platform fee, and payouts. This view never submits route distance or fee
//  arithmetic to that authority.
//
//  Doctrine refs:
...
final class RateSheetStore: ObservableObject {
    enum Pane: String, CaseIterable, Identifiable {
        ...
        var label: String {
            switch self {
            case .calculator: return "Preview"
...
struct MeRateSheet: View {
    ...
    private var legacyPreviewNotice: some View {
        LifecycleCard(accentWarning: true) {
            VStack(alignment: .leading, spacing: 4) {
                Label("Comparison preview — not an executable quote", systemImage: "exclamationmark.shield.fill")
                    .font(EType.caption.weight(.semibold))
                    .foregroundStyle(Brand.warning)
                Text("These editable inputs help compare a Schedule A. Awards, platform fees, payouts, and settlements must read an immutable priced-route version bound to the exact canonical route and committed asset allocation.")
                    .font(EType.caption)
                    .foregroundStyle(palette.textSecondary)
    ...
    private var sheetsPane: some View {
        VStack(spacing: Space.s3) {
            // Canonical commercial authority. This is the only upload path
            // on the visible Truck rate-sheet surface that can ever feed an
            // executable route quote. It stores immutable evidence, exposes
            // blockers, and requires an exact-hash human confirmation.
            PricedRouteRateSheetAuthorityPanel(mode: .truck)
