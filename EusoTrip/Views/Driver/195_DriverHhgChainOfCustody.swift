...
//
//  §W OFFLINE POLICY: ONLINE_ONLY(a custody transfer is a 49 CFR 375 chain-of-custody commit;
//  a queued hand-off would timestamp against the wrong party).
    ...
    private func transferRow(_ t: CustodyTransfer, isLast: Bool) -> some View {
        HStack(alignment: .top, spacing: Space.s3) {
            ...
            if let c = t.cargoCondition, !c.isEmpty {
                // The condition string is the SERVER's verdict on the cargo at
                // hand-off. It was painted Brand.success unconditionally, so a
                // transfer recorded "damaged" / "exception" / "seal_broken"
                // rendered in green. Colour now follows the value.
                Text(c.uppercased()).font(EType.micro).tracking(0.4).fontWeight(.bold)
                    .foregroundStyle(conditionTint(c)).fixedSize()
            }
        }
        .padding(.bottom, isLast ? 0 : Space.s3)
    }

    /// Cargo-condition colour. Only an explicitly good condition earns green;
    /// an explicitly adverse one is warned; anything unrecognised stays
    /// neutral rather than being flattered into success.
    private func conditionTint(_ raw: String) -> Color {
        switch raw.lowercased().replacingOccurrences(of: " ", with: "_") {
        case "good", "intact", "clean", "sealed", "no_exception", "none":
            return Brand.success
        case "damaged", "damage", "exception", "seal_broken", "missing",
             "short", "over", "wet", "torn":
            return Brand.danger
        default:
