...
//
//  §W OFFLINE POLICY: ONLINE_ONLY(a cached transponder state could tell a driver they are
//  green-lit through a scale after the account lapsed).
...
struct TollsPrePassView: View {
    ...
    private func eligibilityCard(_ e: PrePassEligibility) -> some View {
        // Every row here is tri-state. Previously an ABSENT field was coerced
        // to a failing default (`ispScore ?? 0 >= 75`, `csaScore ?? 100 <= 50`,
        // `insuranceCurrent ?? false`) and painted amber "Review" — an adverse
        // verdict on the driver's insurance, registration and scores
        // manufactured from a missing value. Unknown is now its own state.
        //
        // The 75 / 50 band edges are CLIENT thresholds; getPrePassStatus
        // returns no threshold field, so they are labelled as EusoTrip's own
        // read of the FMCSA bands, not asserted as the carrier's official cut.
        VStack(alignment: .leading, spacing: Space.s3) {
            Text("BYPASS ELIGIBILITY").font(EType.micro).tracking(0.6)
                .foregroundStyle(palette.textTertiary)
            eligRow("Safety rating", (e.safetyRating ?? "—").capitalized,
                    ok: e.safetyRating.map { $0.lowercased() == "satisfactory" })
            Divider().overlay(palette.borderFaint)
            eligRow("ISP score", e.ispScore.map { "\($0)" } ?? "—",
                    ok: e.ispScore.map { $0 >= 75 })
            Divider().overlay(palette.borderFaint)
            eligRow("CSA score", e.csaScore.map { "\($0)" } ?? "—",
                    ok: e.csaScore.map { $0 <= 50 })
            Divider().overlay(palette.borderFaint)
            eligRow("Insurance", e.insuranceCurrent.map { $0 ? "Current" : "Review" } ?? "—",
                    ok: e.insuranceCurrent)
            Divider().overlay(palette.borderFaint)
            eligRow("Registration", e.registrationCurrent.map { $0 ? "Current" : "Review" } ?? "—",
                    ok: e.registrationCurrent)
            Text("Bands are EusoTrip's read of the FMCSA ISP/CSA thresholds. A dash means your transponder account did not return that field.")
    ...
    /// `ok == nil` means the server did not report the field — rendered
    /// neutral, never as a pass or a failure.
    private func eligRow(_ label: String, _ value: String, ok: Bool?) -> some View {
        HStack {
            ...
            Text(value).font(EType.mono(.caption)).fontWeight(.bold)
                .foregroundStyle(ok.map { $0 ? Brand.success : Brand.warning }
