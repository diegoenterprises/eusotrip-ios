...
//
//  §W OFFLINE POLICY: ONLINE_ONLY(liveness and the gate PIN are anti-fraud verdicts minted
//  server-side; a replayed or cached pass is the exact fictitious-pickup
//  attack this screen exists to stop).
...
struct DriverAtGateIdentityBindView: View {
    ...
    private var livenessHero: some View {
        ZStack {
            ...
            VStack {
                HStack {
                    // Three states, not two. Before any scan runs the dot was
                    // painted Brand.danger next to the word "READY" — a red
                    // alarm on a neutral state — and a flagged-for-review
                    // verdict was painted Brand.info on the chip while this
                    // dot painted it red, so the two indicators for one state
                    // disagreed. Dot and chip now derive from the same
                    // tri-state and agree in polarity.
                    HStack(spacing: 5) {
                        Circle().fill(livenessTint).frame(width: 7, height: 7)
                        Text(vm.liveness == nil ? "READY" : (vm.livenessOK ? "MATCH" : "REVIEW"))
                    ...
                    Text(matchChip).font(.system(size: 10, weight: .heavy)).tracking(0.4)
                        ...
                        .background(livenessTint, in: Capsule())
                }
                Spacer()
                HStack {
    ...
    /// Tri-state liveness tint. No scan yet is NEUTRAL (not danger); a
    /// flagged verdict is a WARNING the driver must act on (not info); only a
    /// server-confirmed pass is success.
    private var livenessTint: Color {
        guard vm.liveness != nil else { return Brand.info }
        return vm.livenessOK ? Brand.success : Brand.warning
    }
    ...
    private var identityGate: some View {
        ComplianceSection(label: "IDENTITY GATE",
            ...
            VStack(spacing: Space.s3) {
                ...
                // 198B is the full anti-spoof challenge surface — the same
                // `kyc.runLiveness` verdict this row shows, plus the framing
                // viewport, the telemetry breakdown and the driver's prior
                // verification history. It is a SUB-STEP of this gate, so it
                // is reached from here rather than from a Me hub: a liveness
                // scan outside a gate bind has nothing to bind to.
                Button {
                    NotificationCenter.default.post(
                        name: .eusoDriverMeNavSwap, object: nil,
                        userInfo: ["screenId": "198B"]
                    )
                } label: {
                    HStack(spacing: 6) {
                        Image(systemName: "camera.viewfinder")
                            .font(.system(size: 12, weight: .semibold))
                        Text("Open liveness challenge")
