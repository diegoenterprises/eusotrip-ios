...
private struct HosLimit194: Decodable { let remaining: Int? }
private struct HosLimits194: Decodable { let driving: HosLimit194? }
private struct HosSnapshot194: Decodable { let limits: HosLimits194?; let canDrive: Bool? }
...
private final class LivestockViewModel: ObservableObject {
    ...
    func load() async {
        phase = .loading
        do {
            let hos: HosSnapshot194 =
                try await EusoTripAPI.shared.queryNoInput("hos.getCurrentStatus")
            driveRemainingMin = hos.limits?.driving?.remaining
            phase = .ready
        } catch {
            ...
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text(driveClock).font(EType.mono(.body)).fontWeight(.bold)
                        .foregroundStyle((vm.driveRemainingMin ?? 999) < 90 ? Brand.warning : palette.textPrimary)
                }
                Spacer()
                VStack(alignment: .trailing, spacing: 3) {
                    Text("CAN DRIVE").font(EType.micro).tracking(0.6)
                        .foregroundStyle(palette.textTertiary)
                    Text("live · FMCSA").font(EType.mono(.micro)).foregroundStyle(Brand.success)
                }
            }
            ComplianceGapNote(
                systemImage: "timer",
