...
//
//  §W OFFLINE POLICY: ONLINE_ONLY(an anti-spoof verdict computed or replayed on-device is the
    ...
    private var ctaPair: some View {
        HStack(spacing: Space.s3) {
            CTAButton(title: vm.passed ? "Identity confirmed" : "Confirm identity",
                ...
                .disabled(vm.passed)
            // Cancel pops back to 198 (the gate bind that pushed this).
            // `dismiss()` alone was inert here: this screen mounts as a Shell
            // root with no NavigationStack and is never sheet-presented, so a
            // bare DismissAction had nothing to act on and the button did
            // literally nothing on tap.
            Button {
