...
//
//  §W OFFLINE POLICY: ONLINE_ONLY(a cached supply-air temperature is indistinguishable from a
//  live one and could defend a spoilage claim that was never measured; the
//  alert acknowledgement is an FSMA audit-chain write).
...
struct DriverReeferTempScreen: View {
    ...
    var body: some View {
        Shell(theme: theme) {
            // Back pops whatever stack pushed this screen. This previously
            // read `@EnvironmentObject var nav: DriverNavController` and set
            // `nav.currentTab = .trips` — two problems: DriverNavController is
            // never placed in the environment anywhere in the live hierarchy
            // (only in #Preview), so the EnvironmentObject was a fatal-error
            // waiting for the first real presentation; and switching tabs is
            // not "back" when the screen is pushed on the Driver Me stack.
            ReeferTempBody(loadId: loadId, onBack: {
...
private struct ReeferTempBody: View {
    ...
    private var factorTiles: some View {
        HStack(spacing: Space.s3) {
            ...
            // Excursion count is the cargo-safety headline on this screen.
            // Coalescing to 0 rendered an ABSENT count as a reassuring "0
            // excursions" — every other tile here degrades to "-", this one
            // degraded to good news. Absent now reads "-" like the rest.
            MetricTile(label: "Excursions", value: excursionCount.map { "\($0)" } ?? "-",
                       accent: (excursionCount ?? 0) > 0 ? Brand.danger : nil)
        }
    }
    ...
    /// The real excursion count, or nil when neither read answered. Never
    /// coalesced to zero — "no excursions" and "we don't know" are different
    /// claims about the cargo.
    private var excursionCount: Int? { fsma?.excursionCount ?? stats?.excursions }

    private var esangSub: String {
        let pre = (fsma?.preCoolVerified == true) ? "pre-cool signed" : "pre-cool pending"
        guard let ex = excursionCount else {
            return "Excursion count unavailable · FSMA log self-signs at delivery · \(pre)"
        }
    ...
    /// Excursion phrase for the regulatory footer. An absent count is stated
    /// as unavailable rather than printed as "0 active excursions" into a
    /// line that also claims the log is immutable and audit-retained.
    private var excursionFooterPhrase: String {
        guard let ex = excursionCount else { return "excursion count unavailable" }
        return "\(ex) active excursion\(ex == 1 ? "" : "s")"
    }

    private var regulatoryFooter: some View {
        VStack(alignment: .leading, spacing: 3) {
            Text("Reefer telemetry · FSMA 21 CFR 1.908 continuous cold-chain record")
            Text("Load \(loadNumberDisplay) · \(excursionFooterPhrase) · log immutable, audit-retained")
        }
        .font(EType.mono(.micro))
        .foregroundStyle(palette.textTertiary)
        .fixedSize(horizontal: false, vertical: true)
