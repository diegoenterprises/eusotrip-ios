...
private final class AxleScaleViewModel: ObservableObject {
    ...
    var gross: Double { result?.gvw ?? (totalWeight ?? 0) }
    var grossLegal: Bool { gross <= FedLimit.gvw && (result?.compliant ?? false || gross <= FedLimit.gvw) }
...
struct AxleScaleWeighView: View {
    ...
    private var topBar: some View {
        VStack(alignment: .leading, spacing: Space.s3) {
            HStack {
                Text("✦ DRIVER · SCALE & AXLE WEIGHTS")
        ...
        VStack(alignment: .leading, spacing: Space.s3) {
            HStack {
                ...
                Text(grossVerdictText)
                    ...
                    .foregroundStyle(vm.grossLegal ? Brand.success : Brand.danger)
            }
            AxleDiagram(steer: vm.steer, drive: vm.drive, trailer: vm.trailer,
                        awaiting: vm.phase == .awaitingScale)
                .frame(height: 176)
            HStack(spacing: 6) {
                Image(systemName: "scalemass")
                    .font(.system(size: 10, weight: .semibold))
                    .foregroundStyle(palette.textTertiary)
                Text("CAT SCALE · CERTIFIED PLATFORM")
    ...
    private var grossVerdictText: String {
        ...
        let g = Int(vm.gross.rounded())
        return "GROSS \(g.grouped) lb · \(vm.grossLegal ? "LEGAL" : "OVER")"
    }
    ...
    private var coachCard: some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            ...
            Text(vm.grossLegal && (vm.result?.compliant ?? false)
        ...
        VStack(alignment: .leading, spacing: Space.s3) {
            ...
            if let w = vm.lastWeigh {
                HStack(alignment: .top) {
                    ...
                    Spacer()
                    metaCol("GPS", w.lat != nil && w.lng != nil
                            ? String(format: "%.3f, %.3f", w.lat!, w.lng!) : "—")
                }
                HStack(alignment: .top) {
                    metaCol("TIME", shortTime(w.capturedAt))
                    Spacer()
                    metaCol("BY", "Michael Eusorone · ME")
                }
            } else {
