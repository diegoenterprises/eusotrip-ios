...
//
//  §W OFFLINE POLICY: ONLINE_ONLY(a 49 CFR 396.13 pre-trip certification is a signed safety
//  record; it must never be replayed from a queue against a different
//  vehicle, trailer or day).
...
struct DriverPretripDVIRM05Screen: View {
    ...
    var body: some View {
        Shell(theme: theme) {
            // Exit pops whatever stack pushed this screen. This previously
            // read `@EnvironmentObject var nav: DriverNavController` and set
            // `nav.currentTab = .trips` — DriverNavController is never placed
            // in the environment anywhere in the live hierarchy (only in
            // #Preview), so the EnvironmentObject was a fatal-error waiting
            // for the first real presentation.
            M05Body(loadId: loadId, vehicleId: vehicleId, onExit: {
