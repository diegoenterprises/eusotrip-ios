...
private struct RailTenderFailoverBody: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 0) {
                EusoTripEyebrow(verbatim: "SHIPPER · RAIL · TENDER FAILOVER")
    ...
    private var queueSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("No verified fallback rate is available. Candidates are ranked by today's live acceptance rate.")
    ...
    private func reTender() async {
        guard let shipmentId, let n = nextUp else {
            actionNote = "Select a rail shipment before sending the tender to the next eligible carrier."
            return
        }
