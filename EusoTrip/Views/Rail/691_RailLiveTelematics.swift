...
private struct Railcar691: Decodable, Identifiable {
    ...
    var fix: (lat: Double, lng: Double)? {
        ...
        if let coordinate = LatLongParser.validatedCoordinate(
            latitude: yardCoordinates?.lat,
            longitude: yardCoordinates?.lng
        ) {
            return (coordinate.latitude, coordinate.longitude)
        }
        return nil
...
private struct RailLiveTelematicsBody: View {
    ...
    private var eyebrowRow: some View {
        HStack(spacing: 0) {
            EusoTripEyebrow(verbatim: "CARRIER · RAIL · TELEMATICS")
    ...
    // MARK: Position plot — real independent fixes, never inferred track geometry.
    ...
    private var mapHero: some View {
        ...
        let bounds: (minLat: Double, maxLat: Double, minLng: Double, maxLng: Double)? = {
        ...
        return VStack(spacing: 0) {
            HStack {
                Text("POSITION PLOT · TELEMETERED CARS")
            ...
            GeometryReader { g in
                ZStack {
                    // Faint coordinate graticule. This is a relative position
                    // plot, not a track map; only carrier-supplied fixes paint.
                    Path { p in
                    ...
                    if let bounds {
                    // Independent railcar observations must never be sorted by
                    // longitude and joined into a plausible-looking rail line.
                    // Canonical track geometry belongs to the route-plan layer.
        ...
        .accessibilityLabel("Railcar position plot. Independent carrier fixes only. No track geometry is inferred.")
    }
