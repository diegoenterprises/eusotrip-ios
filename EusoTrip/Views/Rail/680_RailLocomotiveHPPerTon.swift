...
private struct RailLocomotiveHPPerTonBody: View {
    ...
    private var eyebrowRow: some View {
        HStack(spacing: 0) {
            Text("✦ RAIL ENGINEER · POWER PLANNING")
    ...
    private var reconciliationRow: some View {
        HStack {
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("grade demand + hp headroom bind on getPowerPlan").font(.system(size: 10, design: .monospaced)).foregroundStyle(palette.textTertiary)
            }
            Spacer()
            VStack(alignment: .trailing, spacing: 2) {
