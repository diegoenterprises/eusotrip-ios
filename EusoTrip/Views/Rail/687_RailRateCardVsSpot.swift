...
private struct RailRateCardVsSpotBody: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 0) {
                EusoTripEyebrow(verbatim: "SHIPPER · RAIL · RATE SOURCE")
    ...
    private var compareSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("No verified spot rate is available. The contract rate card remains the only bookable figure.")
    ...
    private func bookContract() async {
        guard let shipmentId else {
            actionNote = "Select a rail shipment before booking this lane at the contract rate."
            return
        }
