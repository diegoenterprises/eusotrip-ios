...
private struct RailVerifiedReceiverGateBody: View {
    ...
    var body: some View {
        ...
        .alert("Release is a server-gated safety decision", isPresented: $showReleaseNotice) {
            Button("OK", role: .cancel) {}
                    ...
                    Text("A PIH car releases only after EusoTrip revalidates all four receiver checks. With \(checks.count - clearedCount) check\(checks.count - clearedCount == 1 ? "" : "s") pending, the gate remains locked.")
        }
    }

    private var eyebrowRow: some View {
        HStack(spacing: 0) {
            EusoTripEyebrow(verbatim: "CARRIER · RAIL · PIH RELEASE GATE")
    ...
    private var lockNote: some View {
        HStack(spacing: 8) {
            ...
            Text("All four checks are revalidated at release · confirmation required.")
