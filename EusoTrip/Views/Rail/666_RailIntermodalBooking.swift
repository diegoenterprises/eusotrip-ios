...
//  journey hero): a door-to-door segment-order hero (truck→rail→truck,
//  origin/destination, all-in + ETA + distance + CO2), a
...
private struct RailIntermodalBookingBody: View {
    ...
    private var eyebrow: some View {
        HStack {
            EusoTripEyebrow(verbatim: "RAIL ENGINEER · NEW BOOKING")
    ...
    /// The draft knows segment order, rates, and named endpoints, but it has no
    /// canonical road/rail geometry. Present a relay sequence instead of an
    /// authored arc that could be mistaken for the route a solver will commit.
    private var segmentOrder: some View {
        VStack(spacing: 9) {
            HStack {
                Text("SEGMENT ORDER · DRAFT")
                    .font(.system(size: 8, weight: .heavy)).tracking(0.6)
                    .foregroundStyle(palette.textTertiary)
                Spacer()
                Text("NOT ROUTE GEOMETRY")
                    .font(.system(size: 8, weight: .heavy)).tracking(0.5)
                    .foregroundStyle(palette.textTertiary)
            }

            HStack(spacing: 7) {
                endpoint(origin, "shipper dock", color: Brand.blue)
                relayChevron
                ...
                modeNode(icon: "box.truck.fill", gradient: false)
                relayChevron
                endpoint(destination, "consignee", color: Brand.magenta)
            }
        }
        .frame(minHeight: 76)
        .accessibilityElement(children: .combine)
        .accessibilityLabel("Draft segment order from \(origin) to \(destination): truck, rail, truck. Route geometry is not available.")
    }

    private var relayChevron: some View {
