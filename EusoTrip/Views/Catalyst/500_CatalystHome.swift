...
struct CatalystHome: View {
    ...
    enum CatalystWidgetSlot: String, CaseIterable, Codable, Identifiable {
        case activeMatches, recent, news
        var id: String { rawValue }
        var label: String {
            switch self {
            case .activeMatches: return "Active matches"
            case .recent:        return "Recent activity"
            case .news:          return "Catalyst intel"
            }
        }
    }
    @State private var widgetOrder: [CatalystWidgetSlot] = CatalystWidgetSlot.allCases
    @State private var editingLayout: Bool = false
    @State private var dropHoverSlot: CatalystWidgetSlot? = nil
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ...
                // Reorderable secondary-widget zone — parity with
                // Driver 010 / Shipper 200. Persists per-catalyst.
                widgetZoneToolbar
    ...
    // MARK: - Reorderable secondary-widget zone (parity with driver 010 / shipper 200)
    ...
    private var widgetZoneToolbar: some View {
        HStack(spacing: 8) {
            ...
            if editingLayout {
                Button {
                    withAnimation(.easeOut(duration: 0.18)) { widgetOrder = CatalystWidgetSlot.allCases }
                } label: {
                    Text("RESET")
    ...
    @ViewBuilder
    private func secondaryWidget(for slot: CatalystWidgetSlot) -> some View {
        let inner: AnyView = {
            switch slot {
            case .activeMatches: return AnyView(activeMatchesCard)
        ...
        if editingLayout {
            ...
            .dropDestination(for: String.self) { droppedIds, _ in
                guard let raw = droppedIds.first,
                      let dropped = CatalystWidgetSlot(rawValue: raw),
                      dropped != slot,
    ...
    private func hydrateWidgetLayout() async {
        if let data = UserDefaults.standard.data(forKey: widgetLayoutKey),
           let cached = try? JSONDecoder().decode([CatalystWidgetSlot].self, from: data),
           !cached.isEmpty {
        ...
        do {
            let r: Out = try await EusoTripAPI.shared.query("users.getDashboardLayout", input: In(role: "CATALYST"))
            if let server = r.layout, !server.isEmpty {
                let parsed = server.compactMap { CatalystWidgetSlot(rawValue: $0.widgetId) }
                if !parsed.isEmpty {
    ...
    private func persistWidgetLayout() async {
        ...
        do {
            let _: Out = try await EusoTripAPI.shared.mutation(
                ...
                input: In(role: "CATALYST", layout: payload)
            )
        } catch { }
    }

    private func reconcile(_ saved: [CatalystWidgetSlot]) -> [CatalystWidgetSlot] {
        var seen = Set<CatalystWidgetSlot>(); var out: [CatalystWidgetSlot] = []
        for s in saved where !seen.contains(s) { out.append(s); seen.insert(s) }
        for s in CatalystWidgetSlot.allCases where !seen.contains(s) { out.append(s) }
        return out
    }
}
