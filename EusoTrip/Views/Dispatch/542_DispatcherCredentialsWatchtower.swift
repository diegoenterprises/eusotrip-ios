...
private struct MetricProvenance542: Decodable {
    let source: String?
...
private struct MetricTruth542: Decodable {
    let valueState: String
    let accessState: String
    let trackingState: String
    let provenance: MetricProvenance542
    let reason: String?
}

private struct MetricReading542<Value: Decodable>: Decodable {
    let value: Value?
    let truth: MetricTruth542
}

private struct ComplianceReport542: Decodable {
    let overallCompliance: Double?
    let overallComplianceMetric: MetricReading542<Double>
}

private struct RenewalReminder542: Decodable {
    let success: Bool
    let inAppRecorded: Bool
    let emailStatus: String
    let emailFailureKind: String?
}
...
private struct DispatcherCredentialsWatchtowerBody: View {
    ...
    private var topBar: some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            HStack(alignment: .firstTextBaseline) {
                EusoTripEyebrow(verbatim: "DISPATCHER · CREDENTIALS")
    ...
    private func sendReminders() async {
        ...
        struct In: Encodable { let certificationId: String; let requestKey: String }
        var recorded = 0
        var emailDelivered = 0
        var emailFailed = 0
        var emailNotRequested = 0
        var requestFailed = 0
        for c in targets {
            do {
                let out: RenewalReminder542 = try await EusoTripAPI.shared.mutation(
                    "certifications.sendRenewalReminder",
                    input: In(certificationId: c.id, requestKey: UUID().uuidString)
                )
                if out.success && out.inAppRecorded { recorded += 1 }
                switch out.emailStatus {
                case "delivered": emailDelivered += 1
                case "failed": emailFailed += 1
                case "not_requested": emailNotRequested += 1
                default: requestFailed += 1
                }
            } catch {
                requestFailed += 1
            }
        }
        var outcomes = ["Recorded \(recorded) of \(targets.count) in-app"]
        if emailDelivered > 0 { outcomes.append("email delivered \(emailDelivered)") }
        if emailFailed > 0 { outcomes.append("email failed \(emailFailed)") }
        if emailNotRequested > 0 { outcomes.append("email unavailable \(emailNotRequested)") }
        if requestFailed > 0 { outcomes.append("request failed \(requestFailed)") }
        actionNote = outcomes.joined(separator: " · ") + "."
        working = false
    }
    ...
    private func fetchReport() async {
        ...
        do {
            let out: ComplianceReport542 = try await EusoTripAPI.shared.query(
                "certifications.getComplianceReport",
                input: In(entityType: "fleet")
            )
            let metric = out.overallComplianceMetric
            if let value = out.overallCompliance,
               metric.value != nil,
               metric.truth.valueState == "measured",
               metric.truth.trackingState == "tracked",
               metric.truth.accessState == "granted",
               metric.truth.provenance.source != nil {
                actionNote = "Fleet compliance \(Int(value.rounded()))%."
            } else if metric.truth.accessState == "restricted" {
                actionNote = metric.truth.reason ?? "Fleet compliance is restricted for this account."
            } else if metric.truth.valueState == "not_modeled" || metric.truth.trackingState == "not_tracked" {
                actionNote = metric.truth.reason ?? "Fleet compliance is not modeled yet."
            } else {
                actionNote = metric.truth.reason ?? "Fleet compliance has no measured value."
            }
        } catch {
