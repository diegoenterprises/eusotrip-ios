...
private struct DispatcherExceptionTriageBody: View {
    ...
    var body: some View {
        ...
        .task { await load() }
        // RealtimeService → `dispatch:board_update`. Triage is a live
        // queue: an exception cleared or raised elsewhere must leave or
        // enter this list on its own.
    ...
    private var topBar: some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            HStack(alignment: .firstTextBaseline) {
                EusoTripEyebrow(verbatim: "DISPATCHER · EXCEPTIONS · LIVE")
