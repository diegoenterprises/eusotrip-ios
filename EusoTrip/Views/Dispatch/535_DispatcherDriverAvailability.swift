...
private struct RosterUnit: Identifiable, Hashable {
    enum Duty { case available, onLoad, reset }
    let id: Int
    let name: String
    let initials: String
...
private struct DispatcherDriverAvailabilityBody: View {
    ...
    private var units: [RosterUnit] {
        drivers
            ...
            .map { d in
                ...
                let duty = Fleet.duty(status: d.status, hasActiveLoad: active != nil)
                let name = d.name ?? "Unit \(d.id)"
                return RosterUnit(
                    id: d.id,
                    name: name,
                    initials: Fleet.initials(name),
                    sub: Fleet.sub(driver: d, active: active),
                    duty: duty,
                    rightLabel: Fleet.rightLabel(duty: duty, active: active)
                )
            }
    ...
    private func rank(_ d: RosterUnit.Duty) -> Int {
        switch d { case .available: return 0; case .onLoad: return 1; case .reset: return 2 }
    }
    ...
    private var topBar: some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            HStack(alignment: .firstTextBaseline) {
                Text("✦ DISPATCHER · DRIVER AVAILABILITY")
            ...
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("drivers available")
                        ...
                        .foregroundStyle(palette.textPrimary)
                    Text("\(onLoadCount) on a load · \(resetCount) in reset")
                        .font(EType.caption)
    ...
    private var kpiStrip: some View {
        HStack(spacing: Space.s3) {
            ...
            kpiCell("IN RESET",  resetCount,     tint: Brand.hazmat,           filled: false)
        }
...
private enum Fleet {
    static func duty(status: String?, hasActiveLoad: Bool) -> RosterUnit.Duty {
        let s = (status ?? "").lowercased()
        if hasActiveLoad { return .onLoad }
        if s == "off_duty" || s.contains("reset") || s == "on_break" || s == "sleeper" { return .reset }
        if s.contains("driving") || s.contains("on_duty") || s.contains("on_load") { return .onLoad }
        // available / active / anything else with no active load → available
        return .available
    }

    static func sub(driver: FleetDriver, active: FleetLoad?) -> String {
        if let a = active {
            let lane = laneText(a)
    ...
    static func rightLabel(duty: RosterUnit.Duty, active: FleetLoad?) -> String {
        switch duty {
        case .available: return "available"
