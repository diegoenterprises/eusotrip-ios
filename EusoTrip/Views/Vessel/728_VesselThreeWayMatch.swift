    ...
    private var heroCard: some View {
        ZStack(alignment: .topLeading) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text("\(matchedCount) of \(lines.count) lines matched · verified tolerance rules")
    ...
    private var ctaRow: some View {
        HStack(spacing: Space.s3) {
            CTAButton(title: "Dispute \(usd(netVariance))", action: {
                actionMessage = "A verified carrier dispute channel is required before \(usd(netVariance)) can be submitted."
            }, isLoading: varianceCount == 0)
            Button { actionMessage = "Payment approval is unavailable until every line has a verified match." } label: {
