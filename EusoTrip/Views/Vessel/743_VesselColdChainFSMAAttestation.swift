...
private struct VesselColdChainFSMABody: View {
    ...
    private func tempLogSection(_ s: FSMAStatus743) -> some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            Text("TEMPERATURE LOG · FSMA STATUS").font(.system(size: 9, weight: .heavy)).tracking(1.0).foregroundStyle(palette.textTertiary)
    ...
    private var ctaRow: some View {
        HStack(spacing: Space.s2) {
            Button {
                attestNote = "The sanitary-procedures attestation is recorded against the selected booking after confirmation."
            } label: {
