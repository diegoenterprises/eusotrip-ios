...
private struct VesselLashingSecuringPlanBody: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: Space.s5) {
                if loading {
                    ...
                    ctaPair
                    VesselGapNote(text: "Stow context is REAL (getVesselShipmentDetail); CSS Code MSC.1/Circ.1353 is the securing standard. Per-tier securing forces, the peak-MSL % and the ESANG re-lash recommendation need the securing model — proposed vessel.getSecuringPlan / approveSecuringPlan (unsafe-securing detention risk → gated + confirm + audit). No fabricated forces.")
                }
    ...
    private var securingHero: some View {
        VesselHeroCard {
            VStack(alignment: .leading, spacing: Space.s3) {
                ...
                Text("twist-lock / rod / bridge counts await getSecuringPlan")
                    .font(.system(size: 11, weight: .medium))
                    .foregroundStyle(palette.textSecondary)
                    .lineLimit(1).minimumScaleFactor(0.7)
    ...
    private var tierSection: some View {
        VStack(alignment: .leading, spacing: Space.s3) {
            VesselSectionHeader(label: "TIER ELEVATION · SECURING FORCE", right: "STUB · getSecuringPlan")
            VesselGroupCard {
                VStack(spacing: 0) {
        ...
        VesselGroupCard {
            VStack(alignment: .leading, spacing: Space.s2) {
                ...
                Text("Re-lash recommendation loads with securing forces")
                    ...
                    .foregroundStyle(palette.textPrimary)
                    .lineLimit(1).minimumScaleFactor(0.7)
                Text("Once getSecuringPlan reports tier MSL, ESANG proposes the exact re-lash to clear any >90% watch before the departure gate.")
