        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: Space.s5) {
                if loading {
                    ...
                    boardSection
                    if liveUnavailable {
                        VesselGapNote(text: "No live vessels-at-port data was returned. Ports remain ranked by verified capability; congestion and dwell are withheld.")
                    } else {
                        VesselGapNote(text: "Ranking uses live vessels-at-port data. Congestion and average dwell appear only when verified observations are available.")
                    }
                    esang
                    ctaPair
                }
