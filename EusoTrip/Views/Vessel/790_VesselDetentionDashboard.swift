...
private struct VesselDetentionDashboardBody: View {
            ...
            VStack(alignment: .leading, spacing: Space.s4) {
                ...
                if loading {
                    ...
                    EusoEmptyState(systemImage: "shield.lefthalf.filled",
                                   ...
                                   subtitle: "getDetentionDashboard returned no charges in this window. Nothing outstanding, collected, or disputed to show.")
                } else {
                    heroCard
                    kpiStrip
                    facilitiesCard
    ...
    private func viewAll() async {
        ...
        do {
            ...
            actionMessage = "\(hist.total ?? 0) detention events in the current window (getDetentionHistory)."
        } catch {
    ...
    private func exportGap() {
        actionMessage = nil
        actionError = "Ledger export isn't wired yet — detentionAccessorials.exportDetentionLedger is a named server gap. The dashboard reads live; the CSV/PDF writer is pending."
    }
