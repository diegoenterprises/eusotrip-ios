...
private struct VesselConsigneeTrackingLinkBody: View {
    ...
    private var topBar: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Text("✦ VESSEL OPERATOR · TRACKING LINK")
    ...
    private var previewCard: some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            Text("CONSIGNEE PREVIEW · publicTrack")
                .font(.system(size: 9, weight: .heavy)).tracking(1.0).foregroundStyle(palette.textTertiary)
            if link == nil {
    ...
    private var gapNote: some View {
        Text("Active links persist server-side but there is no list-active-links procedure yet — this console shows the link minted this session.")
