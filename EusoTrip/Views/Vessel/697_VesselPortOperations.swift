...
//  Carrier-side live ops console for a marine terminal (the wireframe canon
//  showed Long Beach Pier T / MSC; the build resolves the terminal + identity
//  from live data only). Console/detail grammar — NOT the operator-home grammar:
...
//
//  REAL WIRING (tRPC, server/routers/vesselShipments.ts — re-verified 2026-06-10):
//    · getPorts          {limit:5}  -> resolve home port from live ports rows
//        (id, name, unlocode, totalBerths) — prefer the first row with a real
//        berth count; NO hardcoded city/UNLOCODE seed (vesselShipments.ts:2182)
//    · getBerthSchedule  {portId}  -> vessel_berth_assignments rows ->
//        PORT CALL QUEUE rows + berths-used numerator + ON HOLD count
//        (vesselShipments.ts:1224 -> port_berths/vessel_berth_assignments schema)
//    · getVesselFleet    {limit:50} -> {vessels,total} -> VESSELS in port count
//        (vesselShipments.ts:1377)
//    · getVesselsAtPort  {portId}  -> live MarineTraffic vessels-at-berth, used
//        to enrich the "in port" figure when AIS data is present
//        (vesselShipments.ts:1556)
//
...
//  push the registered berth-window (Vesl698) / port-calls (Vesl661) surfaces
//  via the role-stack nav swap (no fabricated mutation, no dead taps).
//
//  De-fab salvage 2026-06-10 (from PR #50): killed the seeded "MSC" carrier
//  label, the "Long Beach"/USLGB hardcoded home-port resolution, the
//  fabricated "Crane hold" cause on cancelled berth windows, the decorative
//  back chevron, and the two dead CTAs.
//
...
private struct VesselPortOperationsBody: View {
    ...
    /// Top-right mono label — the resolved port's live UNLOCODE (or name), with
    /// an honest em-dash before a port resolves. No fabricated carrier brand.
    private var carrierPortLabel: String {
        if let code = port?.unlocode?.uppercased(), !code.isEmpty { return code }
        if let name = port?.name, !name.isEmpty { return name }
        return "—"
    }
    ...
    private var ctaRow: some View {
        HStack(spacing: Space.s2) {
            // Push the registered Berth Window surface (Vesl698) onto the role
            // stack — same nav-swap mechanism the 656 Me-hub ops rows use.
            Button {
                NotificationCenter.default.post(
                    name: .eusoVesselNavSwap, object: nil,
                    userInfo: ["screenId": "Vesl698"])
            } label: {
                Text("Assign berth")
            ...
            // Lineup = the live Port Calls rotation board (Vesl661).
            Button {
                NotificationCenter.default.post(
                    name: .eusoVesselNavSwap, object: nil,
                    userInfo: ["screenId": "Vesl661"])
            } label: {
                Text("Lineup")
    ...
    private func category(for call: BerthAssignment697) -> CallCategory {
        ...
        case "cancelled":
            // HONEST mapping: the schema has no hold-cause column, so we never
            // fabricate one ("crane", "labor", …) — a cancelled berth window is
            // surfaced as a generic berth hold.
            return CallCategory(title: "Berth hold", badge: "HOLD",
    ...
    private func load() async {
        ...
        do {
            ...
            //    console's denominator); no hardcoded city or UNLOCODE seed.
            let portRows: [PortRow697] = try await EusoTripAPI.shared.query(
                "vesselShipments.getPorts", input: PortsIn(limit: 5, search: nil))
            let resolved = portRows.first { ($0.totalBerths ?? 0) > 0 }
                ?? portRows.first
            self.port = resolved
