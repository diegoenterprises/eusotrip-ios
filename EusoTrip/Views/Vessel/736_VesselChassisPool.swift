    ...
    private var poolUtilizationSection: some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            HStack {
                ...
                Spacer()
                Text("getChassisManagement").font(EType.mono(.caption)).foregroundStyle(palette.textTertiary)
            }
            if poolAggs.isEmpty {
        ...
        VStack(alignment: .leading, spacing: Space.s2) {
            HStack {
                Text("BY TYPE · AVAILABLE")
                    .font(.system(size: 9, weight: .heavy)).tracking(1.0).foregroundStyle(palette.textTertiary)
                Spacer()
                Text("getChassisAvailability").font(EType.mono(.caption)).foregroundStyle(palette.textTertiary)
            }
