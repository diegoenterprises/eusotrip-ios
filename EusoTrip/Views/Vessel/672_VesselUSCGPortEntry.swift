...
private struct VesselUSCGPortEntryBody: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                EusoTripEyebrow(verbatim: "VESSEL OPERATOR · PORT ENTRY")
    ...
    private func fileOrUpdateENOA() async {
        ...
        await load()
        revalidating = false
        actionNote = "Gate verdict revalidated. File the eNOA through NVMC before arrival."
    }
}
