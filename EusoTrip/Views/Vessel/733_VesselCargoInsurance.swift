...
private struct VesselCargoInsuranceBody: View {
    ...
    private var declaredValueControl: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("Operator-declared shipment value · CIF basis · premium recomputed server-side")
                .font(.system(size: 10))
                .foregroundStyle(palette.textTertiary)
        }
        .padding(16)
    ...
    private var quoteSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            sectionLabel("QUOTE · getPerLoadQuote", ref: "insurance.ts:1313", gap: false)
            VStack(spacing: 0) {
                quoteRow(
            ...
            Text("War-risk endorsement available · +0.04% on demand — awaiting getPerLoadQuote war-risk field")
                .font(.system(size: 10))
                .foregroundStyle(palette.textTertiary)
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text("BOOKING · \(vesselLine)")
                    ...
                    .lineLimit(1)
                Spacer()
                Text("insurance.ts:2029")
                    .font(EType.mono(.micro)).foregroundStyle(palette.textTertiary)
            }
            Text(laneLine)
                .font(.system(size: 11)).foregroundStyle(palette.textSecondary)
            Text("shipper-of-record Eusorone Technologies (DU) · generateCOI")
                .font(.system(size: 11)).foregroundStyle(palette.textSecondary)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(16)
