...
private struct VesselOceanFactoringBody: View {
    ...
    private var advanceRateSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            sectionLabel("ADVANCE RATE", ref: "LIVE ADVANCE RATE", gap: false)
    ...
    private var feeWaterfall: some View {
        ...
        .overlay(alignment: .top) {
            sectionLabel("FEE WATERFALL · factoringFeePercent", ref: "VERIFIED FEE SCHEDULE", gap: false)
    ...
    private var debtorCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            sectionLabel("DEBTOR CREDIT · getOverview", ref: "DEBTOR CREDIT PROFILE", gap: false)
            VStack(alignment: .leading, spacing: 10) {
                HStack(spacing: Space.s3) {
                    ZStack {
    ...
    private func requestAdvance() {
        ...
        actionMessage = "Advance terms: \(usd0(a)) today · \(usd0(r)) reserve · \(String(format: "%.1f%%", feeRate * 100)) fee. Funding requires an eligible vessel B/L and approved account."
    }
