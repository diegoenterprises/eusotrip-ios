...
private struct VesselGeneralOrderBody: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: Space.s5) {
                if loading {
                    ...
                    ctaPair
                    VesselGapNote(text: "The 15/45/180-day milestones follow CBP general-order rules. No verified unclaimed-container aging record is linked.")
                }
    ...
    private var atRiskHero: some View {
        VesselHeroCard {
            VStack(alignment: .leading, spacing: Space.s3) {
                ...
                Text("verified aging · importer response status")
    ...
    private var queueSection: some View {
        VStack(alignment: .leading, spacing: Space.s3) {
            VesselSectionHeader(label: "GENERAL-ORDER QUEUE", right: "NO VERIFIED AGING")
            EusoEmptyState(systemImage: "shippingbox.and.arrow.backward",
                           title: "GO queue awaiting the aging model",
                           subtitle: "Unclaimed-container age, importer response, general-order status, and storage appear only from verified custody records.")
