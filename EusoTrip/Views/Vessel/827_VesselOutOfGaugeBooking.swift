...
private struct VesselOOGBookingBody: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: Space.s5) {
                if loading {
                    ...
                    ctaPair
                    VesselGapNote(text: "Booking context is REAL (getVesselShipmentDetail). Over-gauge dimensions, over-limit deltas and stowage sign-off need the OOG model — proposed vessel.createOOGBooking (deterministic over-limit validator → gated + confirm + audit). No fabricated dimensions.")
                }
        ...
        VStack(alignment: .leading, spacing: Space.s3) {
            ...
            VesselGroupCard {
                VStack(alignment: .leading, spacing: Space.s3) {
                    ...
                    Text("Deck envelope 12.19 m (ISO 1496-5) · cargo overhang from getVesselShipmentDetail dims")
    ...
    private var approvalChain: some View {
        VStack(alignment: .leading, spacing: Space.s3) {
            VesselSectionHeader(label: "STOWAGE APPROVAL · CHAIN", right: "STUB · named-gap")
            VesselGroupCard {
                HStack(spacing: 0) {
