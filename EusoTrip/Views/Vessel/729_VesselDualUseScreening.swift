...
private struct VesselDualUseScreeningBody: View {
    ...
    private var classificationChain: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text("No verified HS-to-ECCN classification is available. Unverified control codes remain hidden.")
    ...
    private var gateChecklist: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            .overlay(RoundedRectangle(cornerRadius: Radius.lg, style: .continuous).strokeBorder(palette.borderFaint))
            .clipShape(RoundedRectangle(cornerRadius: Radius.lg, style: .continuous))
            Text("Gate verdicts remain withheld until a verified classifier result is available.")
                .font(.system(size: 10)).foregroundStyle(palette.textTertiary)
        }
    }
    ...
    private var authorityBand: some View {
        VStack(alignment: .leading, spacing: 8) {
            sectionLabel("EXPORT-CONTROL AUTHORITY", ref: "EXPORT-CONTROL CLASSIFICATION", gap: false)
            CountryBand729(rows: [
    ...
    private var ctaRow: some View {
        HStack(spacing: Space.s3) {
            CTAButton(title: "Request license", action: {
                actionMessage = "An authorized export-control result is required before requesting a BIS license or placing a loading hold."
            })
            Button { actionMessage = "Override requires a verified classifier verdict and authorized reviewer." } label: {
