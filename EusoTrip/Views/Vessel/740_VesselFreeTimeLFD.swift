...
private struct VesselFreeTimeLFDBody: View {
    ...
    private var countdownHero: some View {
        ...
        return VStack(alignment: .leading, spacing: Space.s4) {
            HStack {
                Text("LFD COUNTDOWN · getLastFreeDayAlerts")
    ...
    private var ladderSection: some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            HStack {
                ...
                Spacer()
                Text("getFreeTimeManagement").font(EType.mono(.caption)).foregroundStyle(palette.textTertiary)
            }
            VStack(spacing: 0) {
    ...
    private var ctaRow: some View {
        HStack(spacing: Space.s2) {
            Button {
                scheduleNote = "Dray scheduling writes land with createDrayageOrder — pulls are tendered through your drayage provider until then."
            } label: {
                HStack(spacing: 6) {
