...
private struct VesselFeedHealthBody: View {
    ...
    private var carrierRoster: some View {
        VStack(alignment: .leading, spacing: 8) {
            sectionLabel("CARRIER FEED ROSTER", ref: "getOceanTrackingBoard", gap: false)
            VStack(spacing: 0) {
                HStack(spacing: 10) {
                ...
                HStack {
                    ...
                    Spacer()
                    Text("awaiting dcsa.getFeedHealth").font(.system(size: 8.5, weight: .heavy)).foregroundStyle(Brand.warning)
                }
                .padding(.horizontal, 16).padding(.vertical, 12)
            }
    ...
    private var ctaRow: some View {
        HStack(spacing: Space.s3) {
            CTAButton(title: "Reconnect feed", action: {
                actionMessage = "Re-polling a degraded carrier feed awaits dcsa.subscribeFeed (writes vessel.dcsa_feed_reconnect audit + WS broadcast)."
            })
            Button { actionMessage = "Raw feed-event log with per-event latency awaits dcsa.getFeedHealth telemetry." } label: {
