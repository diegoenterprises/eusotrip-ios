...
private struct VesselSlotAllocationBody: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: Space.s5) {
                if loading {
                    ...
                    ctaPair
                    VesselGapNote(text: "Sailings are REAL (blankSailing.dashboard). Per-partner TEU allocation needs the VSA slot model — proposed vessel.getSlotAllocation / vessel.reallocateSlots (revenue-moving → gated + confirm + audit). No fabricated slot figures.")
                }
    ...
    private var partnerSection: some View {
        VStack(alignment: .leading, spacing: Space.s3) {
            VesselSectionHeader(label: "PARTNER ALLOCATION", right: "STUB · getSlotAllocation")
            EusoEmptyState(systemImage: "chart.bar.xaxis",
                           title: "Per-partner slots awaiting the VSA model",
                           subtitle: "Own-vessel, Gemini, slot-buy, swap and spot TEU (allocated vs used vs free) render here once vessel.getSlotAllocation ships. No fabricated capacity.")
        }
    }
