...
private struct VesselMarpolRecordBookBody: View {
    ...
    private var inspectionLine: String {
        ...
        if n == 0 { return "No verified record-book entries are available" }
    ...
    private var annexTabs: some View {
        VStack(alignment: .leading, spacing: 8) {
            sectionLabel("RECORD BOOK · select annex", ref: "VERIFIED RECORD BOOK", gap: true)
            HStack(spacing: 4) {
    ...
    private var entryLog: some View {
        VStack(alignment: .leading, spacing: 8) {
            sectionLabel("OPERATION CODES · MARPOL reference", ref: "OFFICER ENTRY · SEALED RECORD", gap: true)
            VStack(spacing: 0) {
            ...
            Text("No sealed entries are available. The codes above are MARPOL 73/78 references only.")
    ...
    private var pscBand: some View {
        VStack(alignment: .leading, spacing: 8) {
            sectionLabel("PORT-STATE-CONTROL INSPECTION", ref: "COUNTRY COMPLIANCE PROFILE", gap: false)
            CountryBand727(rows: [
    ...
    private var ctaRow: some View {
        HStack(spacing: Space.s3) {
            CTAButton(title: "Add & seal entry", action: {
                actionMessage = "Officer entry and sealing are unavailable until an authorized MARPOL record book is linked."
            })
            Button { actionMessage = "Export is available after at least one sealed record-book entry exists." } label: {
