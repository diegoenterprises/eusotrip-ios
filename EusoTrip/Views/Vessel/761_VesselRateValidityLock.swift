    ...
    private func termsSection(_ l: RateLock761) -> some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            HStack {
                ...
                Spacer()
                Text("STUB · vesselRate.lockQuote").font(.system(size: 8.5, weight: .semibold, design: .monospaced)).foregroundStyle(palette.textTertiary)
            }
            VStack(spacing: 0) {
    ...
    private var ctaPair: some View {
        HStack(spacing: 8) {
            CTAButton(title: "Book at locked rate", action: {
                gapBanner = "Book-at-locked is pending backend · vesselRate.bookLocked (money + irreversible, human-gated) — named gap to the-oath."
