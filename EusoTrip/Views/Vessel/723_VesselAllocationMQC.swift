...
private struct VesselAllocationMQCBody: View {
    ...
    private var weeklyBoard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Booked TEU per sailing · per-sailing allocation cap awaits allocation.weeklySlots")
    ...
    private var ctaRow: some View {
        HStack(spacing: Space.s3) {
            CTAButton(title: "Book next slot", action: {
                actionMessage = "Slot booking against a service-loop allocation awaits allocation.bookSlot (audited, confirm-gated)."
            })
            Button { actionMessage = "MQC fulfilment report needs the contract commitment target from allocation.mqcProgress." } label: {
