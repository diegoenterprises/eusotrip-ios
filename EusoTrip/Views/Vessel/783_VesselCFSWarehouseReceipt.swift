...
private struct VesselCFSWarehouseReceiptBody: View {
            ...
            VStack(alignment: .leading, spacing: Space.s4) {
                ...
                if loading {
                    ...
                    EusoEmptyState(systemImage: "shippingbox",
                                   ...
                                   subtitle: "getWarehouseInventory returned no on-hand items for \(facilityLabel). CFS receipts appear once cargo is received against an ASN.")
                } else {
                    heroCard
                    kpiStrip
                    receiptsCard
    ...
    private var wiringStrip: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text("CFS RECEIPTS · yardManagement").font(.system(size: 9, weight: .heavy)).tracking(0.8)
                    .foregroundStyle(palette.textTertiary)
                Spacer()
                Text("getWarehouseInventory").font(EType.mono(.caption)).foregroundStyle(palette.textSecondary)
            }
