...
private struct VesselTerminalAnalyticsBody: View {
    ...
    private var dwellChartSection: some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            HStack {
                ...
                Spacer()
                Text("DETENTION SUMMARY").font(EType.mono(.caption)).foregroundStyle(palette.textTertiary)
            }
            if ranked.isEmpty {
    ...
    private var summaryRibbon: some View {
        ...
        .overlay(alignment: .topTrailing) {
            Text("LIVE TERMINAL OPERATIONS").font(EType.mono(.micro)).foregroundStyle(palette.textTertiary)
