...
private struct VesselAgenticBody760: View {
    ...
    private var railSection: some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            HStack {
                ...
                Spacer()
                Text("BOOKING SERVICE AVAILABLE").font(.system(size: 8.5, weight: .semibold, design: .monospaced)).foregroundStyle(palette.textTertiary)
            }
            VStack(spacing: 0) {
                ...
                Text("ESANG-assisted rate review · human booking approval required")
    ...
    private func load() async {
        ...
        do {
            ...
            let allIns = rows.compactMap { r -> Int? in
                guard let base = r.ratePerUnit,
                      let baf = r.bafSurcharge,
                      let origin = r.thcOrigin,
                      let destination = r.thcDestination,
                      let peak = r.peakSeasonSurcharge else {
                    return nil
                }
                let sur = baf + origin + destination + peak
