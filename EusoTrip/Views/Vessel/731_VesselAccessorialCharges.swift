...
//  back automatically.
//
//  Docked under SHIPMENTS. transportMode=vessel · USD. (The wireframe canon
//  showed USOAK; the build renders facility/port strings from live rows only.)
//
//  REAL WIRING (tRPC · server/routers/detentionAccessorials.ts — re-verified
//  against the live router 2026-06-10):
...
//    · detentionAccessorials.invoiceDetentionCharge  mutation {claimId}
//        -> flips the approved claim to 'invoiced' + folds the amount into the
//        load's settlement — the REAL "Apply to invoice" verb.  (:1543)
//    · detentionAccessorials.applyAccessorial  mutation
//        {loadId, chargeCode, amount, description, quantity}
//        -> writes ONE NEW accessorial charge row, backing the "Add line"
//        catalog picker only (blockchain audit + WS billing).  (:824)
//
...
//  honest-empty states. "Add line" needs a bound load to write against; when
//  unbound it surfaces the real gap honestly rather than faking a success.
//
//  De-fab salvage 2026-06-10 (from PR #50): "Apply to invoice" now calls the
//  real invoiceDetentionCharge per approved claim (the PR draft re-INSERTED
//  the approved lines via applyAccessorial — silent charge duplication);
//  killed the seeded USOAK port literals; the scope chip reads honestly when
//  no booking is bound; removed the decorative back chevron.
//
...
private struct AccessorialLine731: Identifiable {
    ...
    let meta: String           // "CHC · {facility} · {status}" — live parts only
    let amount: Double
    let state: State
}
...
private struct VesselAccessorialChargesBody: View {
    ...
    /// Booking reference for the eyebrow / hero corner — live load id or an
    /// honest em-dash (no fabricated port code).
    ...
    private var heroCard: some View {
        ZStack(alignment: .topTrailing) {
            VStack(alignment: .leading, spacing: 0) {
                // Scope chips — honest scope label: a bound booking, else the
                // company-wide approved ledger the live proc actually returns.
                HStack(spacing: Space.s2) {
                    chip(loadId > 0 ? "this booking" : "all approved")
    ...
    private func appliedMeta(_ row: AccessorialBillingRow731) -> String {
        ...
        // Facility / origin only when the live row carries one — no seeded port.
    ...
    // MARK: - Apply to invoice (the REAL invoicing verb)
    //
    // invoiceDetentionCharge {claimId} flips each approved detention_claims
    // row (the billable lines ARE those rows — row.id is the claim id) to
    // 'invoiced' and folds the amount into the load's settlement. The PR-#50
    // draft instead re-INSERTED every approved line via applyAccessorial,
    // silently duplicating charges — fixed to the real verb.
    ...
    private func applyToInvoice() async {
        ...
        applying = true
        struct InvoiceIn: Encodable { let claimId: Int }
        struct InvoiceResult: Decodable {
            let success: Bool?
            let invoicedAmount: Double?
        }
        var invoiced = 0
        var invoicedTotal: Double = 0
        var firstError: String? = nil
        for row in (billing?.pendingCharges ?? []) where row.id > 0 {
            do {
                let res: InvoiceResult = try await EusoTripAPI.shared.mutation(
                    "detentionAccessorials.invoiceDetentionCharge",
                    input: InvoiceIn(claimId: row.id))
                if res.success == true {
                    invoiced += 1
                    invoicedTotal += res.invoicedAmount ?? row.amount ?? 0
                }
            } catch {
                if firstError == nil {
        ...
        if invoiced > 0 {
            applyAck = "Invoiced \(invoiced) billable line\(invoiced == 1 ? "" : "s") · \(currency(invoicedTotal))."
            await load()
        }
        if let firstError, invoiced == 0 {
