...
private struct VesselBondedFTZBody: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: Space.s5) {
                if loading {
                    ...
                    ctaPair
                    VesselGapNote(text: "Zone compliance posture is REAL (getVesselCompliance). Duty-in-suspense, the custody split and the admit/withdraw ledger need the FTZ model — proposed vessel.getBondedInventory / vessel.fileWeeklyEntry (irreversible CBP e214 → gated + confirm + audit). No fabricated duty figures.")
                }
    ...
    private var dutyHero: some View {
        VesselHeroCard {
            VStack(alignment: .leading, spacing: Space.s3) {
                ...
                Text("CBP CW-2 class · custody value awaits getBondedInventory")
                    .font(.system(size: 11, weight: .medium))
                    .foregroundStyle(palette.textSecondary)
                    .lineLimit(1).minimumScaleFactor(0.7)
    ...
    private var ledgerSection: some View {
        VStack(alignment: .leading, spacing: Space.s3) {
            VesselSectionHeader(label: "ADMISSIONS / WITHDRAWALS", right: "STUB · getBondedInventory")
            EusoEmptyState(systemImage: "tray.full",
                           title: "Custody ledger awaiting the FTZ model",
                           subtitle: "Each admit (e214 / HTS, +value) and withdraw (CBP 7501 / 7512, −value) posts here once vessel.getBondedInventory ships. No fabricated custody rows.")
