...
private struct VesselInBondMovementBody: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: Space.s5) {
                if loading {
                    ...
                    ctaPair
                    VesselGapNote(text: "CBP disposition is REAL (getCBPEntryStatus / getCBPAlerts) and drives the active ACE node. The IT number, sealed route, seal events and conveyance need the in-bond model — proposed vessel.getInBondStatus / vessel.closeInBond (irreversible CBP arrival report → gated + confirm + audit). No fabricated seal or bond data.")
                }
            ...
            VesselGroupCard {
                VStack(alignment: .leading, spacing: Space.s4) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        Text("Seal integrity · from getInBondStatus")
                            .font(.system(size: 10, weight: .semibold))
                            .foregroundStyle(palette.textSecondary)
                        Spacer(minLength: 6)
    ...
    private var conveyanceSection: some View {
        VStack(alignment: .leading, spacing: Space.s3) {
            VesselSectionHeader(label: "CONVEYANCE & BOND", right: "STUB · getInBondStatus")
            VesselGroupCard {
                HStack(spacing: 0) {
