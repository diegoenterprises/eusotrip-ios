...
private struct VesselCBPExamStationBody: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: Space.s5) {
                if loading {
                    ...
                    ctaPair
                    VesselGapNote(text: "Exam holds are REAL (getCBPAlerts). Exam modality (VACIS/X-ray/intensive/tailgate), CES slot and per-hold fee accrual need the CES model — proposed vessel.getExamStation / scheduleExamSlot (commits CES fee + drayage → gated + confirm + audit). Rows show the real alert type, never a fabricated modality.")
                }
    ...
    private var scannerHero: some View {
        VesselHeroCard {
            VStack(alignment: .leading, spacing: Space.s3) {
                ...
                Text("CES Long Beach · slot + CET referral await getExamStation")
    ...
    private var pipelineSection: some View {
        VStack(alignment: .leading, spacing: Space.s3) {
            VesselSectionHeader(label: "EXAM PIPELINE · CES QUEUE", right: "getCBPAlerts · STUB slots")
            if alerts.isEmpty {
                EusoEmptyState(systemImage: "checkmark.shield",
                               title: "No exam holds in the queue",
                               subtitle: "Open CBP exam holds for this importer post here; exam modality, CES slot and fee arrive with vessel.getExamStation.")
            } else {
                VStack(spacing: 0) {
