    ...
    private var ctaRow: some View {
        HStack(spacing: Space.s3) {
            CTAButton(title: "Surrender EUAs", action: {
                actionMessage = "Surrender of \(eua(obligationEua)) EUA prepared for the \(Int(surrenderPhase * 100))% phase. Irreversible allowance surrender + EUA-price debit await emissions.surrenderAllowances (human-gated)."
            })
            Button { actionMessage = "Pooling surplus intensity across the compliance fleet awaits emissions.fuelEuIntensity pooling." } label: {
