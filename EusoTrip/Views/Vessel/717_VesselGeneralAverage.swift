...
private struct VesselGeneralAverageBody: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                Text("✦ VESSEL OPERATOR · GENERAL AVERAGE").font(.system(size: 9, weight: .heavy)).tracking(1.0)
    ...
    private var apportionmentLedger: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Apportionment computed from the declared GA rate · persistence pending vesselShipments.apportionGeneralAverage")
    ...
    private var fundBar: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let s = stats {
                ...
                Text("Loading marine cover · insurance.getStats…").font(.system(size: 8.5, weight: .semibold)).foregroundStyle(palette.textTertiary)
            }
        }
        .padding(Space.s4).background(palette.bgCard)
        .overlay(RoundedRectangle(cornerRadius: Radius.lg, style: .continuous).strokeBorder(palette.borderFaint, lineWidth: 1))
