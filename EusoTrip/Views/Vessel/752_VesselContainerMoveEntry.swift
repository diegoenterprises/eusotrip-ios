...
private struct VesselContainerMoveEntryBody752: View {
        ...
        ZStack {
            ...
            if let c = ctx {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text("Open this from a container in Positions / Timeline to record its gate-in / gate-out / loaded / discharged event with getContainerTracking context.")
    ...
    private var moveForm: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text("MOVE FORM · recordContainerMovement").font(.system(size: 9, weight: .heavy)).kerning(1.0).foregroundStyle(palette.textTertiary)
                Spacer()
            ...
            VStack(spacing: 0) {
                ...
                formRow(glyph: "clock", tint: Brand.blue, title: "Event time", sub: "now · server timestamp", value: "09:14 PST")
                Divider().background(palette.textPrimary.opacity(0.06)).padding(.leading, 68)
            ...
            .padding(.vertical, 4)
            .background(RoundedRectangle(cornerRadius: 16).fill(palette.bgCard).overlay(RoundedRectangle(cornerRadius: 16).stroke(palette.borderFaint)))
            Text("+ Humidity · + Notes (optional) · mutation writes a containerMovement row + audit")
