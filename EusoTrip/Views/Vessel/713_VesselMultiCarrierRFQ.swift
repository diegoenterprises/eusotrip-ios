...
private struct VesselMultiCarrierRFQBody: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                EusoTripEyebrow(verbatim: "VESSEL OPERATOR · MULTI-CARRIER RFQ")
    ...
    private var ladderSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text("RATE COMPARISON · LIVE CARRIER QUOTES")
                    .font(.system(size: 9, weight: .heavy)).tracking(1.0)
                    .foregroundStyle(palette.textTertiary)
                Spacer()
            ...
            if loading {
                gapCard(title: "Requesting live carrier rates…", detail: "Live carrier quotes · \(originPort) → \(destPort) · \(containerSize)", warn: false)
                ...
                gapCard(title: "\(regime == .ca ? "Vancouver" : "Manzanillo") re-quote pending FX",
                    detail: "Verified all-in quotes are shown in USD. Local-currency conversion is unavailable for this comparison.",
                        warn: false)
            } else {
                VStack(spacing: 0) {
                    HStack {
    ...
    private var ctaPair: some View {
        CTAButton(title: "Refresh carrier quotes", action: { Task { await load() } })
    }
