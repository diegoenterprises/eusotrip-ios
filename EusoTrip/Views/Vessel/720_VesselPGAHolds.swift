...
private struct VesselPGAHoldsBody: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                EusoTripEyebrow(verbatim: "VESSEL OPERATOR · CUSTOMS").font(.system(size: 9, weight: .heavy)).tracking(1.0)
    ...
    private var agencyGates: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Spacer()
                Text("AGENCY HOLD STATUS").font(EType.mono(.micro)).foregroundStyle(palette.textTertiary)
            }
            VStack(spacing: 0) {
    ...
    private var aceStrip: some View {
        HStack(spacing: 12) {
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text("\(alerts.count) verified CBP alert\(alerts.count == 1 ? "" : "s") on file").font(EType.mono(.micro)).foregroundStyle(palette.textSecondary)
            }
            Spacer()
    ...
    private var ctaPair: some View { EmptyView() }
