...
private struct VesselContainerDeconBody819: View {
    ...
    private var topBar: some View {
        VStack(alignment: .leading, spacing: 3) {
            ...
            Text("WASH RACK · LIVE SCHEDULE · FOOD-GRADE RULES")
    ...
    private var typeBoard: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Spacer()
                Text("VERIFIED DECONTAMINATION SCHEDULE").font(EType.mono(.micro)).foregroundStyle(palette.textTertiary)
            }
            VStack(spacing: 0) {
                if types.isEmpty {
    ...
    private var actionRow: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("No decontamination slot can be booked until a verified container and eligible wash bay are selected. Tap Rules for the FSMA changeover matrix.")
