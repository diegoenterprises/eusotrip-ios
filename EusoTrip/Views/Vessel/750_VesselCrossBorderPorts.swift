...
private struct VesselCrossBorderPortsBody750: View {
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                if loading {
                    ...
                    EusoEmptyState(systemImage: "mappin.and.ellipse",
                                   ...
                                   subtitle: "No eligible cross-border gateways were returned for this route.")
                }
    ...
    private func load() async {
        ...
        do {
            ...
            let pacific = ports
                .filter { port in
                    guard let longitude = port.lng,
                          longitude.isFinite,
                          (-180...180).contains(longitude) else { return false }
                    return longitude < -115 && (port.unlocode ?? "") != (heroPort.unlocode ?? "")
                }
