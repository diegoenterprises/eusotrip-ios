...
private struct VesselAutoDetentionRulesBody: View {
            ...
            VStack(alignment: .leading, spacing: Space.s4) {
                ...
                if loading {
                    ...
                    EusoEmptyState(systemImage: "gearshape.2",
                                   ...
                                   subtitle: "getAutoDetentionRules returned no rules for this company. The detection engine has nothing to run.")
                } else {
                    engineHero
                    rulesList
                    ctaPair
    ...
    private var engineHero: some View {
        HStack(alignment: .top, spacing: Space.s4) {
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("30-day capture telemetry pending · getRuleTelemetry gap")
    ...
    private func addRuleGap() {
        actionMessage = nil
        actionError = "New rules are seeded server-side — configureAutoDetention tunes the five existing rules (threshold + auto-claim + on/off). A create endpoint is a proposed server gap."
    }
