...
struct BrokerHome: View {
    ...
    enum BrokerWidgetSlot: String, CaseIterable, Codable, Identifiable {
        case openTenders, recent, news
        var id: String { rawValue }
        var label: String {
            switch self {
            case .openTenders: return "Open tenders"
            case .recent:      return "Recent activity"
            case .news:        return "Broker intel"
            }
        }
    }
    @State private var widgetOrder: [BrokerWidgetSlot] = BrokerWidgetSlot.allCases
    @State private var editingLayout: Bool = false
    @State private var dropHoverSlot: BrokerWidgetSlot? = nil
    ...
    private var widgetZoneToolbar: some View {
        HStack(spacing: 8) {
            ...
            if editingLayout {
                Button {
                    withAnimation(.easeOut(duration: 0.18)) { widgetOrder = BrokerWidgetSlot.allCases }
                } label: {
                    Text("RESET")
    ...
    @ViewBuilder
    private func secondaryWidget(for slot: BrokerWidgetSlot) -> some View {
        let inner: AnyView = {
            switch slot {
            case .openTenders: return AnyView(openTendersCard)
        ...
        if editingLayout {
            ...
            .dropDestination(for: String.self) { droppedIds, _ in
                guard let raw = droppedIds.first,
                      let dropped = BrokerWidgetSlot(rawValue: raw),
                      dropped != slot,
    ...
    private func hydrateWidgetLayout() async {
        if let data = UserDefaults.standard.data(forKey: widgetLayoutKey),
           let cached = try? JSONDecoder().decode([BrokerWidgetSlot].self, from: data),
           !cached.isEmpty {
        ...
        do {
            let r: Out = try await EusoTripAPI.shared.query("users.getDashboardLayout", input: In(role: "BROKER"))
            if let server = r.layout, !server.isEmpty {
                let parsed = server.compactMap { BrokerWidgetSlot(rawValue: $0.widgetId) }
                if !parsed.isEmpty {
    ...
    private func persistWidgetLayout() async {
        ...
        do {
            let _: Out = try await EusoTripAPI.shared.mutation(
                ...
                input: In(role: "BROKER", layout: payload)
            )
        } catch { }
    }

    private func reconcile(_ saved: [BrokerWidgetSlot]) -> [BrokerWidgetSlot] {
        var seen = Set<BrokerWidgetSlot>(); var out: [BrokerWidgetSlot] = []
        for s in saved where !seen.contains(s) { out.append(s); seen.insert(s) }
        for s in BrokerWidgetSlot.allCases where !seen.contains(s) { out.append(s) }
        return out
    }
}
