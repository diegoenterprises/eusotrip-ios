...
//
//  Pure nav hub for the BROKER role — no data/API calls. Each row
//  posts `.eusoBrokerNavSwap` with the destination screen id; the
//  SUPPORT row posts `.eusoBrokereSangTapped` to summon ESANG. Route
//  ids audited against the Broker/ surface files (401 Tenders, 401b
//  Load Board, 405 Active Brokerages, 402b Carrier Vet, 406 Catalyst
//  Vetting, 404 Commission Queue).
//

import SwiftUI

struct BrokerMeScreen: View {
    ...
    private var titleBlock: some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            Text("Brokerage command surface")
                .font(.system(size: 11))
    ...
    private var greeting: String {
        ...
        let name = session.user?.firstName ?? "Broker"
    ...
    //   401  Tenders · 401b Load Board · 405 Active Brokerages
    //   402b Carrier Vet · 406 Catalyst Vetting
...
#Preview("404B · Broker · Me · Dark") {
    BrokerMeScreen(theme: Theme.dark)
        .environmentObject(EusoTripSession())
        .preferredColorScheme(.dark)
}

#Preview("404B · Broker · Me · Light") {
    BrokerMeScreen(theme: Theme.light)
        .environmentObject(EusoTripSession())
