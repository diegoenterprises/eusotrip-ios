...
struct TerminalHome: View {
    ...
    enum TerminalWidgetSlot: String, CaseIterable, Codable, Identifiable {
        case activeMovements, recent, news
        var id: String { rawValue }
        var label: String {
            switch self {
            case .activeMovements: return "Active movements"
            case .recent:          return "Recent activity"
            case .news:            return "Terminal intel"
            }
        }
    }
    @State private var widgetOrder: [TerminalWidgetSlot] = TerminalWidgetSlot.allCases
    @State private var editingLayout: Bool = false
    @State private var dropHoverSlot: TerminalWidgetSlot? = nil
    ...
    private var widgetZoneToolbar: some View {
        HStack(spacing: 8) {
            ...
            if editingLayout {
                Button {
                    withAnimation(.easeOut(duration: 0.18)) { widgetOrder = TerminalWidgetSlot.allCases }
                } label: {
                    Text("RESET")
    ...
    @ViewBuilder
    private func secondaryWidget(for slot: TerminalWidgetSlot) -> some View {
        let inner: AnyView = {
            switch slot {
            case .activeMovements: return AnyView(activeMovementsCard)
        ...
        if editingLayout {
            ...
            .dropDestination(for: String.self) { droppedIds, _ in
                guard let raw = droppedIds.first,
                      let dropped = TerminalWidgetSlot(rawValue: raw),
                      dropped != slot,
    ...
    private func hydrateWidgetLayout() async {
        if let data = UserDefaults.standard.data(forKey: widgetLayoutKey),
           let cached = try? JSONDecoder().decode([TerminalWidgetSlot].self, from: data),
           !cached.isEmpty {
        ...
        do {
            let r: Out = try await EusoTripAPI.shared.query("users.getDashboardLayout", input: In(role: "TERMINAL_MANAGER"))
            if let server = r.layout, !server.isEmpty {
                let parsed = server.compactMap { TerminalWidgetSlot(rawValue: $0.widgetId) }
                if !parsed.isEmpty {
    ...
    private func persistWidgetLayout() async {
        ...
        do {
            let _: Out = try await EusoTripAPI.shared.mutation(
                ...
                input: In(role: "TERMINAL_MANAGER", layout: payload)
            )
        } catch { }
    }

    private func reconcile(_ saved: [TerminalWidgetSlot]) -> [TerminalWidgetSlot] {
        var seen = Set<TerminalWidgetSlot>(); var out: [TerminalWidgetSlot] = []
        for s in saved where !seen.contains(s) { out.append(s); seen.insert(s) }
        for s in TerminalWidgetSlot.allCases where !seen.contains(s) { out.append(s) }
        return out
    }
}
