...
//    • billing.approveDetention executes the finalized one-time wallet payout.
//      It requires a persisted UUID idempotency key and is never called while
//      the fence clock is still running.
...
private struct ShipperDetentionAutoClockBody: View {
    ...
    var body: some View {
        ...
        .eusoRefreshable { await store.load(loadId: loadId) }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(alignment: .top) {
                ...
                Text(store.clock?.headerState ?? "NO CLOCK")
    ...
    private func chargeBuildUpCard(_ c: DetentionClock) -> some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            sectionLabel("SIGNED COMMERCIAL AUTHORITY")
            VStack(spacing: 10) {
                buildRow(title: "Free time", detail: "\(c.freeTimeLabel) negotiated", amount: c.zeroChargeLabel, tone: .neutral)
                Divider().overlay(palette.borderFaint)
                buildRow(title: "Billable detention", detail: c.billableLabel, amount: c.chargeLabel, tone: c.accruing ? .warn : .neutral)
            }
            .padding(Space.s3)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(palette.bgCard)
    ...
    private func esangDisputeCard(_ c: DetentionClock) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                ...
                Spacer(minLength: 0)
                Text(c.isCommerciallyVerified ? "VERIFIED SOURCE" : "REVIEW REQUIRED")
            ...
            Text(c.canPay
                 ? "The finalized charge is ready for one exactly-once wallet payment."
                 : (c.isRunning
                    ? "The final amount locks at fence exit before payment becomes available."
                 : "This charge is \(c.statusLabel.lowercased())."))
    ...
    private func ctaRow(_ c: DetentionClock) -> some View {
        VStack(alignment: .leading, spacing: Space.s2) {
            HStack(spacing: Space.s2) {
                ...
                .buttonStyle(.plain)
                .disabled(c.status == "paid" || store.isPaying)

                CTAButton(
                    title: store.isPaying ? "Paying…" : c.paymentActionLabel,
                    action: { Task { await store.payApprovedCharge() } },
                    isLoading: store.isPaying
                )
                .frame(maxWidth: .infinity)
                .disabled(!c.canPay || store.isPaying)
            }
    ...
    private var footnote: some View {
        Text("Detention is calculated from verified fence timestamps and the agreed free-time grace. Payment becomes available after fence exit; Dispute pauses settlement until your team reviews.")
            .font(EType.caption).foregroundStyle(palette.textTertiary)
...
struct DetentionClock: Equatable {
    ...
    var isRunning: Bool { departureTime == nil && arrivalTime != nil }
    var isCommerciallyVerified: Bool {
        commercialState == "verified_calculation" || commercialState == "verified_live_estimate"
    }
    var accruing: Bool {
        isRunning && commercialState == "verified_live_estimate" && (billableMinutes ?? 0) > 0
    }
    var insideFence: Bool { isRunning && arrivalTime != nil }
    var headerState: String {
        if commercialState == "awaiting_suspension_adjudication" { return "AWAITING ADJUDICATION" }
        if accruing { return "ACCRUING · LIVE" }
        if isRunning { return "CLOCK ACTIVE" }
        return statusLabel.uppercased()
    }
    var heroState: String {
        if accruing { return "DETENTION ACCRUING · VERIFIED LIVE ESTIMATE" }
        if commercialState == "awaiting_suspension_adjudication" {
            return "DETENTION · SUSPENSION REVIEW REQUIRED"
        }
        return isCommerciallyVerified ? "DETENTION · VERIFIED" : "DETENTION · AMOUNT UNAVAILABLE"
    }
    var heroAmountState: String {
        if accruing { return "and climbing" }
        if commercialState == "awaiting_suspension_adjudication" { return "awaiting review" }
        return currentCharge == nil ? "unavailable" : "not accruing"
    }
    var canPay: Bool {
        ["pending_review", "approved"].contains(status ?? "") &&
        departureTime != nil &&
        commercialState == "verified_calculation" &&
        (currentCharge ?? 0) > 0 &&
        currency != nil
    }
    var paymentActionLabel: String {
        if canPay { return "Pay \(chargeLabel)" }
        if isRunning { return "Available at exit" }
    ...
    var facilityDisplay: String {
        let value = facilityName?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return value.isEmpty ? "Facility unavailable" : value
    }
    var statusLabel: String {
        guard let status, !status.isEmpty else { return "Status unavailable" }
        return status.replacingOccurrences(of: "_", with: " ").capitalized
    }
    var chargeLabel: String {
        guard let currentCharge, let currency else { return "—" }
        return currentCharge.formatted(.currency(code: currency.rawValue))
    }
    var zeroChargeLabel: String {
        guard let currency else { return "—" }
        return 0.formatted(.currency(code: currency.rawValue))
    }
    var ratePerHourLabel: String { terms?.rateDisplay ?? "—" }
    var ratePerMinuteLabel: String { terms?.billingDisplay ?? "Signed terms unavailable" }
    var provenanceLabel: String {
        if let sourceReference, !sourceReference.isEmpty {
            return "Source · \(sourceReference)"
        }
        return commercialState.replacingOccurrences(of: "_", with: " ").capitalized
    }

    var enteredLabel: String { humanISO(arrivalTime, format: "HH:mm") }
    var totalDwellClock: String { elapsedMinutes.map(clockString) ?? "—" }
    var freeTimeLabel: String { freeTimeMinutes.map(hoursMinutes) ?? "—" }
    var billableLabel: String { billableMinutes.map(hoursMinutes) ?? "—" }
    var graceExpiredLabel: String {
        guard let a = arrivalTime, let freeTimeMinutes else { return "—" }
    ...
    var heroSubline: String {
        ...
        parts.append(facilityDisplay)
        parts.append(isRunning ? (accruing ? "\(billableLabel) billable" : commercialState.replacingOccurrences(of: "_", with: " ")) : statusLabel)
        return parts.joined(separator: " · ")
    }
    var disputeRead: String {
        switch commercialState {
        case "verified_calculation", "verified_live_estimate":
            return "Verified from \(enteredLabel); signed free time is \(freeTimeLabel). \(terms?.suspensionDisplay ?? "Suspension treatment unavailable")."
        case "awaiting_suspension_adjudication":
            return "The dwell is recorded, but the charge remains unavailable until confirmed suspension time is adjudicated."
        default:
            return "Commercial calculation unavailable. No charge or dispute-risk conclusion is inferred."
        }
    }
...
final class ShipperDetentionAutoClockStore: ObservableObject {
    ...
    func load(loadId raw: String) async {
        ...
        guard let want = targetLoadId(from: raw) else {
            phase = .error("Open the detention clock from a specific load. No other load was selected as a fallback.")
            return
        }
        var activeRows: [DetentionAPI.ActiveDetention] = []
        var historyRows: [DetentionAPI.HistoryEvent] = []
        var failures: [String] = []

        do {
            activeRows = try await EusoTripAPI.shared.detention.getActive(limit: 50).detentions
        } catch {
            failures.append(error.eusoUserCopy)
        }
        do {
            historyRows = try await EusoTripAPI.shared.detention.getHistory(limit: 100).events
        } catch {
            failures.append(error.eusoUserCopy)
        }

        let active = activeRows.first { $0.loadId == want }

        if let picked = active {
            let terms = await signedTerms(for: picked.loadId)
            phase = .clock(DetentionClock(
                ...
                commercialState: picked.commercialState,
                sourceReference: picked.sourceReference,
                sourceHashSha256: picked.sourceHashSha256,
        ...
        let history = historyRows.first { row in
            row.loadId == want
        }
        if let picked = history {
            let terms = await signedTerms(for: picked.loadId)
            phase = .clock(DetentionClock(
                id: picked.id,
                loadId: picked.loadId,
                ...
                elapsedMinutes: picked.totalMinutes,
                freeTimeMinutes: picked.freeTimeMinutes,
                billableMinutes: picked.billableMinutes,
                currentCharge: picked.totalCharge,
                currency: picked.currency,
                commercialState: picked.commercialState,
                sourceReference: picked.sourceReference,
                sourceHashSha256: picked.sourceHashSha256,
                status: picked.status ?? picked.billingStatus,
                terms: terms
            ))
            return
        }
        ...
        if failures.count == 2 {
            phase = .error(failures.joined(separator: "\n"))
        } else {
            phase = .empty
        }
    }

    private func signedTerms(for loadId: Int?) async -> TruckDetentionNegotiatedTerms? {
        guard let loadId else { return nil }
        do {
            return try await EusoTripAPI.shared.loads.getDetail(id: String(loadId))?.truckDetentionTerms
        } catch {
            actionError = "Signed truck detention terms could not be loaded. \(error.eusoUserCopy)"
            return nil
        }
    }
    ...
    /// Executes the finalized, exactly-once wallet payment after fence exit.
    func payApprovedCharge() async {
        guard let c = clock, c.canPay else {
            actionError = "This detention charge is not ready for payment."
            return
        }
        isPaying = true
        actionError = nil
        ...
        do {
            let result = try await EusoTripAPI.shared.detention.approveDetention(
                detentionId: c.id,
                idempotencyKey: paymentIdempotencyKey(for: c.id)
            )
            guard result.success, result.status == "paid" else {
                throw EusoTripAPIError.trpcError("The detention payment did not reach a paid state.")
            }
            await load(loadId: requestedLoadId)
        } catch {
            actionError = error.eusoUserCopy
        }
    }

    private func paymentIdempotencyKey(for detentionId: Int) -> UUID {
        let storageKey = "billing.detention.\(detentionId).payment-idempotency-key"
        if let existing = UserDefaults.standard.string(forKey: storageKey),
           let uuid = UUID(uuidString: existing) {
            return uuid
        }
        let uuid = UUID()
        UserDefaults.standard.set(uuid.uuidString.lowercased(), forKey: storageKey)
        return uuid
    }
}
...
// here as a collision-free extension so this per-load payout fires the real
// exactly-once write. Server input mirrors billing.ts verbatim.
...
extension DetentionAPI {
    ...
    @discardableResult
    func approveDetention(
        detentionId: Int,
        adjustedAmount: Double? = nil,
        idempotencyKey: UUID
    ) async throws -> ApproveDetentionResult {
