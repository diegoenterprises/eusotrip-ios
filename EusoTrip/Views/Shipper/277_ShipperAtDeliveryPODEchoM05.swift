//
//  277_ShipperAtDeliveryPODEchoM05.swift
//  EusoTrip — Shipper · LIFECYCLE ECHO · AT-DELIVERY · POD (M-05 chain).
//
//  Wireframe: 02 Shipper/Dark-SVG/277 Shipper At-Delivery POD Echo M05.svg
//  Archetype: MAP/TRACKING + delivery confirmation. Shows Diego the moment
//  his load reaches the consignee with the POD signed, so he can confirm
//  the packet lines up before approving paperwork.
//
...
//      PAPERWORK stage, not fired here — matches the SVG's §418 note.)
//    • Message   → messaging (310) via nav-swap.
//  RBAC: shipperProcedure. transportMode=truck · US.
//
//  Honest: piece-count is not on the POD packet — the card surfaces the
//  real receiver + signed timestamp instead of a fabricated "4/4 pieces".
//
...
struct ShipperAtDeliveryPODEchoM05Screen: View {
        ...
        Shell(theme: theme) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ShipperEchoSnapshotView(loadId: loadId, eyebrow: "SHIPPER · LOAD · DELIVERED · POD") { live in
                    DeliveryPODM05Body(live: live, loadId: loadId)
                }
            }
...
private struct DeliveryPODM05Body: View {
...
#Preview("277 · At-delivery POD · Night") {
    ShipperAtDeliveryPODEchoM05Screen(theme: Theme.dark, loadId: "0")
        .environmentObject(EusoTripSession()).preferredColorScheme(.dark)
}
#Preview("277 · At-delivery POD · Afternoon") {
    ShipperAtDeliveryPODEchoM05Screen(theme: Theme.light, loadId: "0")
