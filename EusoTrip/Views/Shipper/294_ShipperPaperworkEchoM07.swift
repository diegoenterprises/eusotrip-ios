//
//  294_ShipperPaperworkEchoM07.swift
//  EusoTrip — Shipper · LIFECYCLE ECHO · PAPERWORK (M-07 chain).
//
//  Wireframe: 02 Shipper/Dark-SVG/294 Shipper Paperwork Echo M07.svg
//  Archetype: MONEY / RECONCILIATION. The M-07 dialect of the paperwork
//  echo (a dry-van general-cargo lane LA → Phoenix): BOL, POD, and
//  accessorials line up against the awarded amount so Diego approves and
//  stages the settlement in one tap. A money-led hero + a reconciliation
//  ledger (BOL / POD) + a settlement breakdown. Closes the M-07 lifecycle
//  loop at the PAPERWORK port; the dry-van-vs-reefer context is REAL
//  snapshot data (cargoType / weight), not baked copy.
//
...
struct ShipperPaperworkEchoM07Screen: View {
        ...
        Shell(theme: theme) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ShipperEchoSnapshotView(loadId: loadId, eyebrow: "SHIPPER · LOAD · PAPERWORK") { live in
                    PaperworkEchoM07Body(live: live, loadId: loadId)
                }
            }
...
private struct PaperworkEchoM07Body: View {
...
#Preview("294 · Paperwork echo M07 · Night") {
    ShipperPaperworkEchoM07Screen(theme: Theme.dark, loadId: "0")
        .environmentObject(EusoTripSession()).preferredColorScheme(.dark)
}
#Preview("294 · Paperwork echo M07 · Afternoon") {
    ShipperPaperworkEchoM07Screen(theme: Theme.light, loadId: "0")
