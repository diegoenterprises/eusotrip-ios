//
//  284_ShipperInTransitEchoM06.swift
//  EusoTrip — Shipper · LIFECYCLE ECHO · IN-TRANSIT (M-06 chain).
//
//  Wireframe: 02 Shipper/Dark-SVG/284 Shipper In-Transit Echo M06.svg
//  Archetype: MAP/TRACKING. The M-06 dialect of 276 (a reefer produce lane
//  running I-75 south). Diego watches his one awarded reefer move on a live
//  map so "where is my produce and is it on time?" is answered in three
//  seconds without a carrier-portal login. (The SVG's §-marker / snapshot-
//  source dev copy is not reproduced — clean golden echo atoms only.)
//
...
//
//  Honest gap (mirrors 276_InTransitEchoM05): the snapshot exposes no server
...
//  it does NOT fabricate a "410 mi / 38%" figure.
//
...
struct ShipperInTransitEchoM06Screen: View {
        ...
        Shell(theme: theme) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ShipperEchoSnapshotView(loadId: loadId, eyebrow: "SHIPPER · LOAD · IN-TRANSIT") { live in
                    InTransitM06Body(live: live, loadId: loadId)
                }
            }
...
private struct InTransitM06Body: View {
...
#Preview("284 · In-transit M06 · Night") {
    ShipperInTransitEchoM06Screen(theme: Theme.dark, loadId: "0")
        .environmentObject(EusoTripSession()).preferredColorScheme(.dark)
}
#Preview("284 · In-transit M06 · Afternoon") {
    ShipperInTransitEchoM06Screen(theme: Theme.light, loadId: "0")
