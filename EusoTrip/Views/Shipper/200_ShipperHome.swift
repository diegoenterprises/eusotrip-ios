...
//
//  First brick on the Shipper role track (200s). Replaces the
//  `RolePlaceholderScreen` stub the dev chrome was rendering for
//  `ProductionScreen.Role.shipper`. Pixel-doctrine compliant per
//  EUSOTRIP2027GOLD §2 (gradient-only accent, GradientToggleStyle
//  on every Toggle — none on this brick), §4 (tokenized spacing /
//  radius / type), §5 (palette semantic only, no hard-coded
//  Color.white / Color.black / Color.gray fills), §7 (`AnyShapeStyle`
//  wrapping for ternary shape-styles), §10 (previews compile in
//  isolation).
//
//  Cohort B — fully dynamic (SKILL.md §3 "no-mock" pledge · 2027
//  motivation "no fake data"):
//
//    • KPI strip → `shippers.getDashboardStats` via
//      `ShipperDashboardStore` (LiveDataStores.swift). Server
//      returns a six-figure envelope: activeLoads, pendingBids,
//      deliveredThisWeek, ratePerMile, onTimeRate,
//      totalSpendThisMonth — MCP-verified at
//      `frontend/server/routers/shippers.ts:77`.
//    • "Needs your attention" alert strip →
//      `shippers.getLoadsRequiringAttention` via
//      `ShipperAlertsStore` (shippers.ts:147). Empty until the
//      server flags a stale or pending-bid load.
//    • "Active loads" feed → `shippers.getActiveLoads` via
//      `ShipperActiveLoadsStore` (shippers.ts:109).
//    • "Recent activity" feed → `shippers.getRecentLoads` via
//      `ShipperRecentLoadsStore` (shippers.ts:191).
...
struct ShipperHome: View {
    ...
    private var subhead: String {
        if let outer = dashboard.state.value, let s = outer {
            ...
            let bids = s.pendingBids
            return "\(active) active load\(active == 1 ? "" : "s") · \(bids) pending bid\(bids == 1 ? "" : "s")"
        }
        return "Loading shipment fabric…"
    }
        ...
        case .loaded(let maybe):
            if let s = maybe {
                ...
                EusoEmptyState(
                    ...
                    title: "No KPIs yet",
                    subtitle: "Post your first load and the dashboard will populate the moment a bid lands."
                )
            }
        case .empty:
            EusoEmptyState(
    ...
    private func kpiGrid(_ s: ShipperAPI.DashboardStats) -> some View {
        LazyVGrid(columns: [GridItem(.flexible(), spacing: Space.s2),
                            GridItem(.flexible(), spacing: Space.s2)],
                  spacing: Space.s2) {
            kpiTile(label: "ACTIVE LOADS",     value: "\(s.activeLoads)",            sub: "in flight now")
            kpiTile(label: "PENDING BIDS",     value: "\(s.pendingBids)",            sub: "awaiting review")
            kpiTile(label: "DELIVERED · 7D",   value: "\(s.deliveredThisWeek)",      sub: "completed this week")
            kpiTile(label: "MONTH SPEND",      value: dollars(s.totalSpendThisMonth), sub: "MTD")
        }
    }
    ...
    private func alertRow(_ row: ShipperAPI.LoadAlert) -> some View {
    ...
    private func activeRow(_ row: ShipperAPI.ActiveLoad) -> some View {
