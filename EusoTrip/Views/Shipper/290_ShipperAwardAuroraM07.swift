//
//  290_ShipperAwardAuroraM07.swift
//  EusoTrip — Shipper · LIFECYCLE ECHO · AWARDED (M-07 chain).
//
//  Wireframe: 02 Shipper/Dark-SVG/290 Shipper Award Aurora M07.svg
//  Archetype: DETAIL + money-led outcomes ledger. The M-07 dialect (a
//  dry-van general-cargo lane LA → Phoenix). Closes the bidding loop in
//  one decisive view: who won, what was saved (or paid over) against
//  target, and the underbidders surfaced with the amount each lost by.
//  The favorite-over-cheapest award decision is the shipper's real intent;
//  this screen reports the outcome from the live bids, never fabricates it.
//
//  Web peer: frontend/client/src/pages/shipper/loads/:id (awarded state).
//  Wiring (on-disk confirmed):
//    • shippers.getLifecycleSnapshot EXISTS shippers.ts:1216 → awarded
//      carrier + posted rate + lane.
//    • shippers.getBidsForLoad       EXISTS shippers.ts:761 → the outcomes
//      ledger (awarded + lost quotes).
//    • View tender → load detail (205) via nav-swap (rate-con / tender pane).
//    • Message RM  → messaging (310) via nav-swap.
//      NOTE dispatcher.comms.openThread is a STUB on the web peer (named-
//      gap surfaced to the-oath); this routes to the real in-app messaging
...
struct ShipperAwardAuroraM07Screen: View {
        ...
        Shell(theme: theme) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ShipperEchoSnapshotView(loadId: loadId, eyebrow: "SHIPPER · LOAD · AWARDED") { live in
                    AwardM07Body(live: live, loadId: loadId)
                }
            }
...
private struct AwardM07Body: View {
...
#Preview("290 · Award M07 · Night") {
    ShipperAwardAuroraM07Screen(theme: Theme.dark, loadId: "0")
        .environmentObject(EusoTripSession()).preferredColorScheme(.dark)
}
#Preview("290 · Award M07 · Afternoon") {
    ShipperAwardAuroraM07Screen(theme: Theme.light, loadId: "0")
