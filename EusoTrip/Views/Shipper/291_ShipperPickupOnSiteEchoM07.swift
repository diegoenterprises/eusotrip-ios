//
//  291_ShipperPickupOnSiteEchoM07.swift
//  EusoTrip — Shipper · LIFECYCLE ECHO · PICKUP · ON-SITE (M-07 chain).
//
//  Wireframe: 02 Shipper/Dark-SVG/291 Shipper Pickup On-Site Echo M07.svg
//  Archetype: MAP/TRACKING + on-site status. The M-07 dialect (a dry-van
//  general-cargo lane at the LA origin dock). Shows Diego the moment his
//  awarded Aurora driver reaches the origin dock so he can confirm pickup
//  is on time at a glance.
//
//  Web peer: frontend/client/src/pages/shipper/loads/:id.
//  Wiring (on-disk confirmed):
//    • shippers.getLifecycleSnapshot EXISTS shippers.ts:1216 → carrier/
//      driver, origin geofence (truck-at-pickup pin + dwell), lane, rate.
...
//  RBAC: shipperProcedure. transportMode=truck · US (dry van · dock-to-dock).
//
...
struct ShipperPickupOnSiteEchoM07Screen: View {
        ...
        Shell(theme: theme) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ShipperEchoSnapshotView(loadId: loadId, eyebrow: "SHIPPER · LOAD · PICKUP · ON-SITE") { live in
                    PickupOnSiteM07Body(live: live, loadId: loadId)
                }
            }
...
private struct PickupOnSiteM07Body: View {
...
#Preview("291 · Pickup on-site M07 · Night") {
    ShipperPickupOnSiteEchoM07Screen(theme: Theme.dark, loadId: "0")
        .environmentObject(EusoTripSession()).preferredColorScheme(.dark)
}
#Preview("291 · Pickup on-site M07 · Afternoon") {
    ShipperPickupOnSiteEchoM07Screen(theme: Theme.light, loadId: "0")
