//
//  280_ShipperFreshMatrixPostedM06.swift
//  EusoTrip — Shipper · LIFECYCLE ECHO · POSTED (M-06 chain).
//
//  Wireframe: 02 Shipper/Dark-SVG/280 Shipper Fresh Matrix Posted M06.svg
//  Archetype: DETAIL — the calm landing page in the first hour after a post.
//  The M-06 dialect of 272 (a reefer produce lane instead of flatbed steel):
...
//    • shippers.getLifecycleSnapshot EXISTS shippers.ts → hero rate + window
//      + bids counter + spec (PRIMARY CONSUME).
...
//
//  Honest gap: the SVG's invited-catalyst roster (AUR/CCE/ERN with
//  favorite flags + win-rates + NOTIFIED chips) is not on the snapshot — the
//  panel renders the REAL incoming quotes and an honest awaiting-state
//  instead of a fabricated favorites list.
//
...
struct ShipperFreshMatrixPostedM06Screen: View {
        ...
        Shell(theme: theme) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ShipperEchoSnapshotView(loadId: loadId, eyebrow: "SHIPPER · LOAD · POSTED") { live in
                    PostedM06Body(live: live, loadId: loadId)
                }
            }
...
private struct PostedM06Body: View {
            ...
            VStack(alignment: .leading, spacing: 10) {
                HStack(alignment: .top, spacing: 12) {
                    ZStack {
                        ...
                        Image(systemName: "snowflake").font(.system(size: 16, weight: .semibold)).foregroundStyle(Brand.rail)
                    }
...
#Preview("280 · Posted M06 · Night") {
    ShipperFreshMatrixPostedM06Screen(theme: Theme.dark, loadId: "0")
        .environmentObject(EusoTripSession()).preferredColorScheme(.dark)
}
#Preview("280 · Posted M06 · Afternoon") {
    ShipperFreshMatrixPostedM06Screen(theme: Theme.light, loadId: "0")
