//
//  279_ShipperClosedSettlementPaidEchoM05.swift
//  EusoTrip — Shipper · LIFECYCLE ECHO · CLOSED / SETTLEMENT-PAID (M-05).
//
//  Wireframe: 02 Shipper/Dark-SVG/279 Shipper Closed Settlement Paid Echo M05.svg
//  Archetype: MONEY / RECEIPT (terminal, read-only). A tall gradient-rimmed
//  receipt hero leads with the paid amount, the saving against target, and a
//  chain-complete footline, then a full settlement breakdown. Terminal read
//  surface — the pay action ran upstream (founder-initiated per the no-
//  execute-money guardrail); this screen only confirms the receipt.
//
...
//    • shippers.getLifecycleSnapshot EXISTS shippers.ts → closed load +
...
//
//  Honest fan-out (per the SVG desc): at the terminal paid state the shipper
//  is NOT a `paid` push recipient — this surface learns the closed/paid
//  state via getSettlementForLoad + getLifecycleSnapshot re-poll + the WS
//  LOAD_STATE_CHANGE broadcast, not a Payment-Received push. The "saved vs
//  target" delta is computed from the real settlement amount and posted rate;
//  the SVG's method line renders the settlement `source` verbatim, never a
//  fabricated rail.
//
...
struct ShipperClosedSettlementPaidEchoM05Screen: View {
        ...
        Shell(theme: theme) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ShipperEchoSnapshotView(loadId: loadId, eyebrow: "SHIPPER · LOAD · CLOSED · PAID") { live in
                    ClosedPaidM05Body(live: live, loadId: loadId)
                }
            }
...
private struct ClosedPaidM05Body: View {
...
#Preview("279 · Closed settled M05 · Night") {
    ShipperClosedSettlementPaidEchoM05Screen(theme: Theme.dark, loadId: "0")
        .environmentObject(EusoTripSession()).preferredColorScheme(.dark)
}
#Preview("279 · Closed settled M05 · Afternoon") {
    ShipperClosedSettlementPaidEchoM05Screen(theme: Theme.light, loadId: "0")
