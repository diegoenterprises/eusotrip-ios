//
//  289_ShipperBidReviewM07.swift
//  EusoTrip — Shipper · LIFECYCLE ECHO · BIDDING (M-07 chain).
//
//  Wireframe: 02 Shipper/Dark-SVG/289 Shipper Bid Review M07.svg
//  Archetype: DETAIL + bid leaderboard. The M-07 dialect (a dry-van
//  general-cargo lane LA → Phoenix). Shows Diego the live board mid-window
//  — the cheapest quote at the top with a safety grade he can trust, his
//  favorite carrier ranked below so the choice is never just price, and
//  one tap to accept the leader.
//
//  Web peer: frontend/client/src/pages/shipper/loads/:id?tab=bids.
//  Wiring (on-disk confirmed):
//    • shippers.getLifecycleSnapshot EXISTS shippers.ts:1216 → hero + window.
//    • shippers.getBidsForLoad       EXISTS shippers.ts:761 → the leaderboard
//      (catalyst-decorated: name, USDOT, safetyScore, amount, recommended).
//    • Accept leader → shippers.acceptBid EXISTS shippers.ts:409 (awards +
//      drops the other bids; transitions loads.status → assigned).
...
//  safetyScore. No fabricated win-rate percentages or "typing" indicators.
//
...
struct ShipperBidReviewM07Screen: View {
        ...
        Shell(theme: theme) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ShipperEchoSnapshotView(loadId: loadId, eyebrow: "SHIPPER · LOAD · BIDDING") { live in
                    BidReviewM07Body(live: live, loadId: loadId)
                }
            }
...
private struct BidReviewM07Body: View {
...
#Preview("289 · Bid review M07 · Night") {
    ShipperBidReviewM07Screen(theme: Theme.dark, loadId: "0")
        .environmentObject(EusoTripSession()).preferredColorScheme(.dark)
}
#Preview("289 · Bid review M07 · Afternoon") {
    ShipperBidReviewM07Screen(theme: Theme.light, loadId: "0")
