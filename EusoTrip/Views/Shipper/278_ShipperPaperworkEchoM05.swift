//
//  278_ShipperPaperworkEchoM05.swift
//  EusoTrip — Shipper · LIFECYCLE ECHO · PAPERWORK (M-05 chain).
//
//  Wireframe: 02 Shipper/Dark-SVG/278 Shipper Paperwork Echo M05.svg
//  Archetype: MONEY / RECONCILIATION. Distinct from the 270 M-04 paperwork
//  echo: this is the one screen where BOL, POD, and accessorials line up
//  against the awarded amount so Diego can approve and stage the settlement
//  in a single tap. Money-led hero + a reconciliation ledger (BOL / POD) +
//  a settlement breakdown, NOT a settlement-strip + KPI-quartet skeleton.
//
...
//      carrier + lane + status (PRIMARY CONSUME).
//    • shippers.getSettlementForLoad EXISTS shippers.ts → staged amount,
//      status, payable date (CONSUMED · staged payable to the carrier).
//    • Approve POD & settle → pod.approvePOD EXISTS pod.ts (PRIMARY ACTION ·
//      flips pod_pending → delivered, staging the settlement) then routes to
//      the settlement detail (227) via nav-swap.
...
//  accessorialTotal; the staged amount is the real settlement.amount (falls
//  back to the awarded load.rate). No fabricated line items — an absent
//  settlement renders the awarded amount and an em-dash payable.
//
...
struct ShipperPaperworkEchoM05Screen: View {
        ...
        Shell(theme: theme) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ShipperEchoSnapshotView(loadId: loadId, eyebrow: "SHIPPER · LOAD · PAPERWORK") { live in
                    PaperworkEchoM05Body(live: live, loadId: loadId)
                }
            }
...
private struct PaperworkEchoM05Body: View {
...
#Preview("278 · Paperwork echo M05 · Night") {
    ShipperPaperworkEchoM05Screen(theme: Theme.dark, loadId: "0")
        .environmentObject(EusoTripSession()).preferredColorScheme(.dark)
}
#Preview("278 · Paperwork echo M05 · Afternoon") {
    ShipperPaperworkEchoM05Screen(theme: Theme.light, loadId: "0")
