//
//  285_ShipperAtDeliveryPODEchoM06.swift
//  EusoTrip — Shipper · LIFECYCLE ECHO · AT-DELIVERY · POD (M-06 chain).
//
//  Wireframe: 02 Shipper/Dark-SVG/285 Shipper At-Delivery POD Echo M06.svg
//  Archetype: MAP/TRACKING + delivery confirmation. The M-06 dialect of 277
//  (a reefer produce lane at the consignee). Shows Diego the moment his load
//  reaches the consignee with the POD signed, so he can confirm the packet
//  lines up before approving paperwork. (The SVG's §-marker / snapshot-source
//  dev copy is not reproduced — clean golden echo atoms only.)
//
//  Web peer: frontend/client/src/pages/shipper/loads/:id (POD packet view).
//  Wiring (on-disk confirmed):
//    • shippers.getLifecycleSnapshot EXISTS shippers.ts → delivered state,
//      arrival geofence, carrier/driver, rate.
//    • pod.getPODForLoad             EXISTS pod.ts       → POD packet
//      (receiver, submittedAt, status, notes).
//    • Review POD → POD receipt (248) via nav-swap. (Approval happens at the
//      PAPERWORK stage / 278, not fired here.)
//    • Message   → messaging (310) via nav-swap.
//  RBAC: shipperProcedure. transportMode=truck · US (reefer · seal + temp log).
//
//  Honest: piece/pallet-count and seal-intact are not on the POD packet —
//  the card surfaces the real receiver + signed timestamp instead of a
//  fabricated "24/24 pallets · seal intact".
//
...
struct ShipperAtDeliveryPODEchoM06Screen: View {
        ...
        Shell(theme: theme) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ShipperEchoSnapshotView(loadId: loadId, eyebrow: "SHIPPER · LOAD · DELIVERED · POD") { live in
                    DeliveryPODM06Body(live: live, loadId: loadId)
                }
            }
...
private struct DeliveryPODM06Body: View {
...
#Preview("285 · At-delivery POD M06 · Night") {
    ShipperAtDeliveryPODEchoM06Screen(theme: Theme.dark, loadId: "0")
        .environmentObject(EusoTripSession()).preferredColorScheme(.dark)
}
#Preview("285 · At-delivery POD M06 · Afternoon") {
    ShipperAtDeliveryPODEchoM06Screen(theme: Theme.light, loadId: "0")
