//
//  292_ShipperInTransitEchoM07.swift
//  EusoTrip — Shipper · LIFECYCLE ECHO · IN-TRANSIT (M-07 chain).
//
//  Wireframe: 02 Shipper/Dark-SVG/292 Shipper In-Transit Echo M07.svg
//  Archetype: MAP/TRACKING. The M-07 dialect (a dry-van general-cargo lane
//  running I-10 eastbound toward Phoenix). Diego watches his one awarded
//  Aurora load move on a live map so "where is my freight and is it on
//  time?" is answered in three seconds without a carrier-portal login.
//
//  Web peer: frontend/client/src/pages/shipper/loads/:id (live-tracking).
//  Wiring (on-disk confirmed):
//    • shippers.getLifecycleSnapshot EXISTS shippers.ts:1216 → status, ETA
...
//
//  Honest gap (mirrors the M-05/M-06 in-transit echoes): the snapshot
//  exposes no server "miles completed / % progress / speed" scalar. TRIP
//  PULSE therefore renders ETA, TOTAL distance, LAST PING age, and STATUS
//  from real fields — it does NOT fabricate a "210 mi / 43%" figure.
//
...
struct ShipperInTransitEchoM07Screen: View {
        ...
        Shell(theme: theme) {
            VStack(alignment: .leading, spacing: Space.s4) {
                ShipperEchoSnapshotView(loadId: loadId, eyebrow: "SHIPPER · LOAD · IN-TRANSIT") { live in
                    InTransitM07Body(live: live, loadId: loadId)
                }
            }
...
private struct InTransitM07Body: View {
...
#Preview("292 · In-transit M07 · Night") {
    ShipperInTransitEchoM07Screen(theme: Theme.dark, loadId: "0")
        .environmentObject(EusoTripSession()).preferredColorScheme(.dark)
}
#Preview("292 · In-transit M07 · Afternoon") {
    ShipperInTransitEchoM07Screen(theme: Theme.light, loadId: "0")
