...
struct CarrierHome: View {
    ...
    enum CarrierWidgetSlot: String, CaseIterable, Codable, Identifiable {
        case activeLoads, recent, news
        var id: String { rawValue }
        var label: String {
            switch self {
            case .activeLoads: return "Active loads"
            case .recent:      return "Recent activity"
            case .news:        return "Carrier intel"
            }
        }
    }
    @State private var widgetOrder: [CarrierWidgetSlot] = CarrierWidgetSlot.allCases
    @State private var editingLayout: Bool = false
    @State private var dropHoverSlot: CarrierWidgetSlot? = nil
    ...
    private var widgetZoneToolbar: some View {
        HStack(spacing: 8) {
            ...
            if editingLayout {
                Button {
                    withAnimation(.easeOut(duration: 0.18)) { widgetOrder = CarrierWidgetSlot.allCases }
                } label: {
                    Text("RESET")
    ...
    @ViewBuilder
    private func secondaryWidget(for slot: CarrierWidgetSlot) -> some View {
        let inner: AnyView = {
            switch slot {
            case .activeLoads: return AnyView(activeLoadsCard)
        ...
        if editingLayout {
            ...
            .dropDestination(for: String.self) { droppedIds, _ in
                guard let raw = droppedIds.first,
                      let dropped = CarrierWidgetSlot(rawValue: raw),
                      dropped != slot,
    ...
    private func hydrateWidgetLayout() async {
        if let data = UserDefaults.standard.data(forKey: widgetLayoutKey),
           let cached = try? JSONDecoder().decode([CarrierWidgetSlot].self, from: data),
           !cached.isEmpty {
        ...
        do {
            let r: Out = try await EusoTripAPI.shared.query("users.getDashboardLayout", input: In(role: "CARRIER"))
            if let server = r.layout, !server.isEmpty {
                let parsed = server.compactMap { CarrierWidgetSlot(rawValue: $0.widgetId) }
                if !parsed.isEmpty {
    ...
    private func persistWidgetLayout() async {
        ...
        do {
            let _: Out = try await EusoTripAPI.shared.mutation(
                ...
                input: In(role: "CARRIER", layout: payload)
            )
        } catch { }
    }

    private func reconcile(_ saved: [CarrierWidgetSlot]) -> [CarrierWidgetSlot] {
        var seen = Set<CarrierWidgetSlot>(); var out: [CarrierWidgetSlot] = []
        for s in saved where !seen.contains(s) { out.append(s); seen.insert(s) }
        for s in CarrierWidgetSlot.allCases where !seen.contains(s) { out.append(s) }
        return out
    }
}
